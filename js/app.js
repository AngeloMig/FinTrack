...
function deleteTransfer(id){const t=transfers.find(x=>x.id===id);if(!t)return;adjustAccountBalance(t.from,t.amount);adjustAccountBalance(t.to,-t.amount);transfers=transfers.filter(x=>x.id!==id);saveData();render()}
...
function markRecurringPaid(id){const item=recurring.find(x=>x.id===id);if(!item)return;const monthKey=currentMonthKey();if(item.lastPaid===monthKey)return alert('Already marked as paid this month.');if(item.type==='bill'){entries.unshift(stampRecord({id:nextId++,date:todayStr,category:item.category||'Miscellaneous / Buffer',amount:item.amount,note:'Recurring: '+item.name,account:getDefaultAccountKey()}));adjustAccountBalance(getDefaultAccountKey(),-item.amount)}else{incomes.unshift(stampRecord({id:nextIncId++,date:todayStr,source:item.name,amount:item.amount,note:'Recurring income',account:getDefaultAccountKey()}));adjustAccountBalance(getDefaultAccountKey(),item.amount)}item.lastPaid=monthKey;saveData();render()}
...
function updateAddPreviews(){
  const expWrap=document.getElementById('expense-balance-preview');
  const expAcc=document.getElementById('f-account')?.value||'';
  const expAmount=parseFloat(document.getElementById('f-amount')?.value)||0;
  const expBtn=document.getElementById('tour-target-submit-expense');
  if(expWrap){
    const current=Number(nwBalances[expAcc]||0);
    const after=current-expAmount;
    const hasEnough=expAmount<=current;
    expWrap.innerHTML=`<div class="add-subtitle" style="margin-bottom:4px">Expense balance preview</div><div><strong>${esc(getAccountInfo(expAcc).name)}</strong> now: ${fmt(current)}</div><div>After this expense: <strong>${fmt(after)}</strong></div>${expAmount>0?(hasEnough?'<div class="add-balance-ok">Enough balance for this expense.</div>':'<div class="add-balance-warning">Not enough balance in this account.</div>'):''}`;
    if(expBtn){
      const canSubmit = expAmount>0 && !!expAcc && hasEnough;
      expBtn.disabled = !canSubmit;
      expBtn.textContent = hasEnough || expAmount<=0 ? 'Add Expense' : 'Not Enough Balance';
    }
  }
...
/* Export & Backup */
function getHistoryState(){return{search:(document.getElementById('hist-search')?.value||'').trim().toLowerCase(),category:document.getElementById('hist-filter')?.value||'all',month:document.getElementById('hist-month')?.value||'',type:document.getElementById('hist-type')?.value||'all',account:document.getElementById('hist-account')?.value||'all',sort:document.getElementById('hist-sort')?.value||'newest',min:parseFloat(document.getElementById('hist-min')?.value),max:parseFloat(document.getElementById('hist-max')?.value)}}
...
function resetHistoryFilters(){['hist-search','hist-min','hist-max'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});['hist-filter','hist-type'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='all'});document.getElementById('hist-sort').value='newest';document.getElementById('hist-account').value='all';const monthEl=document.getElementById('hist-month');if(monthEl)monthEl.value=filterMonth;render()}
function exportCSV(){const hs=getHistoryState();let expenseData=[...entries];let incomeData=[...incomes];if(hs.month){expenseData=expenseData.filter(e=>e.date.startsWith(hs.month));incomeData=incomeData.filter(i=>i.date.startsWith(hs.month))}if(hs.category&&hs.category!=='all')expenseData=expenseData.filter(e=>e.category===hs.category);if(hs.type==='expense')incomeData=[];if(hs.type==='income')expenseData=[];if(!expenseData.length&&!incomeData.length)return alert('No data.');const rows=[];if(expenseData.length){rows.push(['--- EXPENSES ---']);rows.push(['Date','Category','Account','Amount','Note']);rows.push(...expenseData.map(e=>[e.date,`"${e.category}"`,`"${getAccountInfo(e.account).name}"`,e.amount,`"${e.note||''}"`]))}if(incomeData.length){rows.push(['--- INCOME ---']);rows.push(['Date','Source','Account','Amount','Note']);rows.push(...incomeData.map(i=>[i.date,`"${i.source}"`,`"${getAccountInfo(i.account).name}"`,i.amount,`"${i.note||''}"`]))}const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([rows.map(r=>r.join(',')).join('\n')],{type:'text/csv'}));a.download=`finance-${hs.month||'all'}.csv`;a.click()}
...
const TOOLTIP_CONTENT={
  ...
  history:{title:'History & Filters',body:'Search and filter expenses or income by month, account, amount, and type. You can also export filtered data as CSV.'},
...
function getDebtPayoffData(remaining, forecast, needsBudget){
  const monthlyMinimum=debts.reduce((s,d)=>s+Number(d.payment||0),0);
...
function render(){
  ...
  const cw=getWeek(now);const weekTotal=entries.filter(e=>{const d=new Date(e.date);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&getWeek(d)===cw}).reduce((s,e)=>s+e.amount,0);
  ...
  const slide0 = `
    <div class="greeting-slide ${greetingCardIndex===0?'active':''}">
      ...
      <div class="greeting-progress">
        ...
        <div class="greeting-progress-meta"><span>Spent ${fmtShort(monthTotal)} of ${fmtShort(totalIncome)}${carryoverOverspend>0?` · Carryover ${fmtShort(carryoverOverspend)}`:''}</span><span>${Math.round(spentPct)}% used</span></div>
      </div>
    </div>`;
  const slide1 = `
  ...
  // === HISTORY ===
  const hf=document.getElementById('hist-filter');const curF=hf.value||'all';
  hf.innerHTML='<option value="all">All Categories</option>'+ac.map(c=>`<option value="${esc(c.name)}">${esc(c.name)}</option>`).join('');hf.value=curF;
  ...
  let filtEnt=[...entries];let filtInc=[...incomes];
  if(histState.category!=='all')filtEnt=filtEnt.filter(e=>e.category===histState.category);
  if(histState.month){filtEnt=filtEnt.filter(e=>e.date.startsWith(histState.month));filtInc=filtInc.filter(i=>i.date.startsWith(histState.month))}
  if(histState.account&&histState.account!=='all'){filtEnt=filtEnt.filter(e=>e.account===histState.account);filtInc=filtInc.filter(i=>i.account===histState.account)}
  if(!Number.isNaN(histState.min)){filtEnt=filtEnt.filter(e=>e.amount>=histState.min);filtInc=filtInc.filter(i=>i.amount>=histState.min)}
  if(!Number.isNaN(histState.max)){filtEnt=filtEnt.filter(e=>e.amount<=histState.max);filtInc=filtInc.filter(i=>i.amount<=histState.max)}
  if(histState.search){filtEnt=filtEnt.filter(e=>matchesHistorySearch({kind:'expense',...e},histState.search));filtInc=filtInc.filter(i=>matchesHistorySearch({kind:'income',...i},histState.search))}
  sortHistoryItems(filtEnt,histState.sort);sortHistoryItems(filtInc,histState.sort);
  const historyCards=[];
  if(histState.type!=='income')historyCards.push(...filtEnt.map(e=>({kind:'expense',...e})));
  if(histState.type!=='expense')historyCards.push(...filtInc.map(i=>({kind:'income',...i})));
  sortHistoryItems(historyCards,histState.sort);
  const expenseTotal=filtEnt.reduce((s,e)=>s+e.amount,0);const incomeTotal=filtInc.reduce((s,i)=>s+i.amount,0);
  document.getElementById('history-summary').innerHTML=`<div class="stats-grid" style="margin-bottom:12px"><div class="stat-card"><div class="stat-label">Results</div><div class="stat-value" style="font-size:18px">${historyCards.length}</div></div><div class="stat-card"><div class="stat-label">Totals</div><div class="stat-value" style="font-size:16px;color:var(--red)">-${fmtShort(expenseTotal)}</div><div class="stat-change" style="color:var(--green)">+${fmtShort(incomeTotal)}</div></div></div>`;
  const hcEl=document.getElementById('history-content');
  ...
  // === DEBTS ===
  document.getElementById('debt-summary').innerHTML=debts.length?`<div style="display:grid;gap:10px;margin-bottom:14px"><div style="display:flex;gap:10px"><div style="flex:1;padding:12px;background:var(--red-soft);border-radius:var(--radius-xs);text-align:center"><div style="font-size:11px;color:var(--text3)">Total Owed</div><div style="font-size:18px;font-weight:800;color:var(--red)">${fmtShort(totalDebt)}</div></div><div style="flex:1;padding:12px;background:var(--surface2);border-radius:var(--radius-xs);text-align:center"><div style="font-size:11px;color:var(--text3)">Monthly Payments</div><div style="font-size:18px;font-weight:800">${fmtShort(debtPayoffData.monthlyMinimum)}</div></div></div><div class="card" style="padding:14px;margin:0"><div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;margin-bottom:10px">Payoff method</div><div class="quick-actions" style="margin-bottom:10px"><button class="quick-chip" style="${debtPayoffSettings.method==='minimum'?'border-color:var(--accent);color:var(--accent);background:var(--accent-soft)':''}" onclick="setDebtPayoffMethod('minimum')">Minimum Only</button><button class="quick-chip" style="${debtPayoffSettings.method==='snowball'?'border-color:var(--accent);color:var(--accent);background:var(--accent-soft)':''}" onclick="setDebtPayoffMethod('snowball')">Snowball</button><button class="quick-chip" style="${debtPayoffSettings.method==='avalanche'?'border-color:var(--accent);color:var(--accent);background:var(--accent-soft)':''}" onclick="setDebtPayoffMethod('avalanche')">Avalanche</button></div><div style="font-size:13px;font-weight:700">${debtPayoffData.targetDebt?`Focus on ${esc(debtPayoffData.targetDebt.name)}`:'Pay minimums on all debts'}</div><div style="font-size:12px;color:var(--text2);margin-top:4px">${debtPayoffData.extraPayment>0&&debtPayoffData.targetDebt?`Suggested extra payment: ${fmtShort(debtPayoffData.extraPayment)}`:'No safe extra payment right now'}</div></div></div>`:'';
  document.getElementById('debt-list').innerHTML=debts.length?debts.map(d=>{const mo=d.payment>0?Math.ceil(d.total/d.payment):Infinity;const isTarget=debtPayoffData.targetDebt&&debtPayoffData.targetDebt.id===d.id;const strategyBadge=debtPayoffSettings.method==='minimum'?'Minimum':debtPayoffSettings.method==='avalanche'?'Avalanche target':'Snowball target';const isPaid=d.lastPaidMonth===currentMonthKey();const paymentSummary=getDebtPaymentSummary(d.id);const latest=paymentSummary.latest;return`<div class="debt-card"><div style="display:flex;justify-content:space-between;margin-bottom:4px;gap:8px"><div><span style="font-weight:700">${esc(d.name)}</span> <span style="font-size:11px;color:var(--text3)">${esc(d.type)}</span>${isTarget?` <span style="font-size:10px;font-weight:700;color:var(--accent);background:var(--accent-soft);padding:3px 8px;border-radius:999px;display:inline-block;margin-top:4px">${strategyBadge}</span>`:''}${isPaid?` <span style="font-size:10px;font-weight:700;color:var(--green);background:var(--green-soft);padding:3px 8px;border-radius:999px;display:inline-block;margin-top:4px">Paid this month</span>`:''}</div><div style="font-weight:700;color:var(--red)">${fmt(d.total)}</div></div><div style="font-size:12px;color:var(--text3)">${fmt(d.payment)}/mo ${d.interest?`· ${d.interest}% APR`:''} ${d.due?`· Due: ${esc(d.due)}`:''} · ${d.payment>0?`~${mo} mo`:'—'}</div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><button class="btn btn-sm btn-primary" onclick="event.stopPropagation();openDebtPayment(${d.id})">💳 Log Payment</button><button class="btn btn-sm btn-ghost" onclick="event.stopPropagation();openDebtEdit(${d.id})">Edit</button><button class="btn btn-sm btn-ghost" onclick="event.stopPropagation();openDebtHistory(${d.id})">View History</button></div><div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)"><div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px">Payment summary</div>${paymentSummary.count?`<div style="display:grid;gap:4px;font-size:12px;color:var(--text2)"><div><strong>Last:</strong> ${esc(formatDateTime(latest))} — ${fmt(latest.amount)}</div><div><strong>Total paid:</strong> ${fmt(paymentSummary.total)}</div><div><strong>${paymentSummary.count} payment${paymentSummary.count!==1?'s':''}</strong></div></div>`:`<div style="font-size:12px;color:var(--text3)">No payments yet</div>`}</div></div>`}).join(''):'<div class="empty"><div class="empty-icon">🎉</div><div class="empty-text">No debts — keep it up!</div></div>';
