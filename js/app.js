...
const moneyInputValue=n=>roundMoney(n).toFixed(2);
...
let _ftHandler=null;
const _DIALOG_IDS=new Set(['modal-app-alert','modal-app-confirm']);
function _setupFocusTrap(id){
  const ov=document.getElementById(id);if(!ov)return;
  if(_ftHandler){document.removeEventListener('keydown',_ftHandler);_ftHandler=null;}
  const sel='a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])';
  setTimeout(()=>{const f=ov.querySelector(sel);if(f)f.focus();},80);
  _ftHandler=e=>{
    if(e.key==='Escape'){closeModal(id);return;}
    if(e.key!=='Tab')return;
    const els=[...ov.querySelectorAll(sel)].filter(el=>el.offsetParent!==null);
    if(!els.length)return;
    const first=els[0],last=els[els.length-1];
    if(e.shiftKey){if(document.activeElement===first){e.preventDefault();last.focus();}}
    else{if(document.activeElement===last){e.preventDefault();first.focus();}}
  };
  document.addEventListener('keydown',_ftHandler);
}
function _teardownFocusTrap(){
  if(_ftHandler){document.removeEventListener('keydown',_ftHandler);_ftHandler=null;}
}
function _ensureModalCloseButton(overlay){
  if(!overlay)return;
  const modal=overlay.querySelector(':scope > .modal');
  if(!modal)return;
  if(modal.classList.contains('modal-no-close'))return;
  if(modal.querySelector(':scope > .modal-close-btn'))return;
  const id=overlay.id;
  if(!id)return;
  ...
  btn.title='Close';
  btn.innerHTML='<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6l12 12M18 6L6 18"/></svg>';
  btn.addEventListener('click',function(e){e.stopPropagation();closeModal(id);});
  modal.insertBefore(btn,modal.firstChild);
}
function openModal(id){
  if(!_DIALOG_IDS.has(id)){
    const prev=document.querySelector('.modal-overlay.show');
    if(prev&&prev.id!==id&&!_DIALOG_IDS.has(prev.id))prev.classList.remove('show');
  }
  const overlay=document.getElementById(id);
  if(overlay&&overlay.classList.contains('modal-overlay'))_ensureModalCloseButton(overlay);
  overlay.classList.add('show');
  document.body.classList.add('modal-open');
  if(id==='modal-add-wish')setTimeout(updateWishPlanPreview,0);
  _setupFocusTrap(id);
}
const _modalListeners={};
function addModalListener(modalId,el,event,fn,opts){
  if(!_modalListeners[modalId])_modalListeners[modalId]=[];
  el.addEventListener(event,fn,opts);
  _modalListeners[modalId].push({el,event,fn});
}
function _cleanupModalListeners(id){
  (_modalListeners[id]||[]).forEach(({el,event,fn})=>el.removeEventListener(event,fn));
  delete _modalListeners[id];
}
function closeModal(id){
  document.getElementById(id).classList.remove('show');
  _teardownFocusTrap();
  _cleanupModalListeners(id);
  const remaining=document.querySelector('.modal-overlay.show');
  if(remaining){_setupFocusTrap(remaining.id);}
...
function undoLastTransfer(){if(!lastTransferUndo)return;const t=lastTransferUndo.transfer;adjustAccountBalance(t.from,lastTransferUndo.totalDeduction);if(!isExternalTransfer(t))adjustAccountBalance(t.to,-t.amount);transfers=transfers.filter(x=>x.id!==t.id);if(lastTransferUndo.feeEntryId){entries=entries.filter(e=>e.id!==lastTransferUndo.feeEntryId)}if(lastTransferUndo.outflowEntryId){entries=entries.filter(e=>e.id!==lastTransferUndo.outflowEntryId)}lastTransferUndo=null;saveData();render();showActionToast('Transfer undone','The account balances were restored.','Undo')}
...
function openBudgetHub(){
  if(typeof showTab==='function')showTab('more');
  requestAnimationFrame(()=>{
    const hub=document.getElementById('budgets-hub');
...
function getCashflowTimelineMockData(){
  ...
  const baseBalance=getTrackedAssetBalance()||0;
...
function getMonthSpent(monthKey=filterMonth){
  return roundMoney(getMonthEntries(monthKey).reduce((sum,e)=>sum+Number(e.amount||0),0));
}
function getMonthIncomeTotal(monthKey=filterMonth){
  return roundMoney(getMonthIncome(monthKey).reduce((sum,i)=>sum+Number(i.amount||0),0));
}
...
function getMonthCategoryTotals(monthKey=filterMonth){
  ...
  getMonthEntries(monthKey).filter(isBudgetTrackedEntry).forEach(e=>{
    ...
    totals[e.category]=roundMoney((totals[e.category]||0)+Number(e.amount||0));
  });
  return totals;
}
...
function getBudgetProgressItems(categories=allCats(),monthKey=filterMonth){
  ...
  return categories.filter(c=>c.group!=='savings').map(c=>{
    ...
    const{base,rollover,total:budget}=getEffectiveBudget(c.name,monthKey);
    const pct=budget>0?(spent/budget)*100:0;
    return {...c,spent,budget,base,rollover,pct,over:isBudgetOver(spent,budget)};
  });
}
...
function renderAnalyticsSummary(){
  ...
  wrap.innerHTML=`
    ...
    <div class="ms-hero-grid">
      <div class="ms-hero-block">
        <div class="ms-hero-label">Spent this month</div>
        <div class="ms-hero-val">${fmt(monthSpent)}</div>
        <div class="ms-hero-sub">Avg ${fmt(avgDaily)}/day in ${monthLabel}</div>
      </div>
      <div class="ms-hero-block">
        <div class="ms-hero-label">Left to spend</div>
        <div class="ms-hero-val ${remValCls}">${fmt(remaining)}</div>
        <div class="ms-hero-sub">${remSub}</div>
      </div>
    </div>
    <div class="ms-bottom-row">
      <div class="ms-bottom-item">
        <div class="ms-bottom-label">Daily limit</div>
        <div class="ms-bottom-val">${fmt(safeDaily)}</div>
        <div class="ms-bottom-sub">÷ ${safe.daysLeft} day${safe.daysLeft===1?'':'s'} left</div>
      </div>
      <div class="ms-bottom-divider"></div>
      <div class="ms-bottom-item">
        <div class="ms-bottom-label">Status</div>
        <div style="margin-top:4px"><span class="sum-status-badge ${statusClass}">${monthlyStatus}</span></div>
        <div class="ms-bottom-sub">${statusClass==='good'?'Pace is healthy':statusClass==='warn'?'Stay controlled':'Limit spending'}</div>
      </div>
    </div>
  `;
}
...
const MONEY_FLOW_PALETTES={
  income:['#10b981','#22c55e','#14b8a6','#34d399','#84cc16','#5eead4'],
  account:['#64748b','#475569','#94a3b8','#334155','#52525b','#71717a'],
  spending:['#ef4444','#f97316','#eab308','#06b6d4','#ec4899','#14b8a6','#3b82f6','#a855f7','#84cc16','#f59e0b','#0ea5e9','#e11d48'],
  savings:['#6366f1','#8b5cf6','#a855f7']
};
function getPreviousMonthKey(monthKey){const[y,m]=String(monthKey||'').split('-').map(Number);if(!y||!m)return monthKey;const d=new Date(y,m-2,1);return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}
function getMoneyFlowShortMonthLabel(monthKey){return new Date(`${monthKey}-01T00:00:00`).toLocaleDateString('en-PH',{month:'short'})}
...
function buildMoneyFlowData(monthKey=currentMonthKey()){
  ...
  const sourceAccountDates=new Map();
  const accountCategoryDates=new Map();
  const dayFromDate=ds=>{const d=parseInt(String(ds||'').slice(8,10),10);return Number.isFinite(d)&&d>=1&&d<=31?d:15};
  ...
  monthIncomes.forEach(income=>{
    ...
    const dKey=`${source}|||${account}`;
    const ds=sourceAccountDates.get(dKey)||{sum:0,weight:0};
    ds.sum+=dayFromDate(income.date)*amount;
    ds.weight+=amount;
    sourceAccountDates.set(dKey,ds);
  });
  ...
  monthEntries.forEach(entry=>{
    ...
    const accInfo=getAccountInfo(account);
    accountMeta.set(account,{key:account,label:accInfo.name,icon:accInfo.icon||'🏦'});
    const stat=categoryStats.get(categoryMeta.label)||{count:0,topAmount:0,topNote:'',entries:[]};
    stat.count++;
    if(amount>stat.topAmount){stat.topAmount=amount;stat.topNote=String(entry.note||entry.category||'').trim();}
    stat.entries.push({amount,date:entry.date||'',note:String(entry.note||'').trim(),account,category:categoryMeta.label});
    categoryStats.set(categoryMeta.label,stat);
    const dKey=`${account}|||${categoryMeta.label}`;
    const ds=accountCategoryDates.get(dKey)||{sum:0,weight:0};
    ds.sum+=dayFromDate(entry.date)*amount;
    ds.weight+=amount;
    accountCategoryDates.set(dKey,ds);
  });
  ...
  const keptCategoryLabels=new Set([
    ...spendingCategories.slice(0,8).map(cat=>cat.label),
    ...savingsCategories.slice(0,4).map(cat=>cat.label)
  ]);
  const groupedCategories=new Map();
  const groupedCategoryStats=new Map();
  ...
  rawCategoryTotals.forEach((rawCat,rawLabel)=>{
    const targetLabel=keptCategoryLabels.has(rawLabel)?rawLabel:'Other';
    const stat=categoryStats.get(rawLabel)||{count:0,topAmount:0,topNote:'',entries:[]};
    const agg=groupedCategoryStats.get(targetLabel)||{count:0,topAmount:0,topNote:'',entries:[]};
    agg.count+=stat.count;
    if(stat.topAmount>agg.topAmount){agg.topAmount=stat.topAmount;agg.topNote=stat.topNote;}
    agg.entries=[...(agg.entries||[]),...(stat.entries||[])];
    groupedCategoryStats.set(targetLabel,agg);
  });
  groupedCategoryStats.forEach(agg=>{
    if(Array.isArray(agg.entries)){
      agg.entries.sort((a,b)=>Number(b.amount||0)-Number(a.amount||0));
      if(agg.entries.length>5)agg.entries.length=5;
    }
  });
  ...
  const accountRealIncomeMap=new Map([...accountInTotals.entries()].map(([k,v])=>[k,Number(v.amount||0)]));
  ...
  const sources=[...sourceTotals.values()].sort((a,b)=>b.amount-a.amount).map(source=>({id:`source:${source.label}`,label:source.label,icon:source.icon,total:Number(source.amount||0),color:source.label==='Existing Balance'?'#94a3b8':MONEY_FLOW_PALETTES.income[(incomeColorIdx++)%MONEY_FLOW_PALETTES.income.length]}));
  const accounts=[...accountMeta.values()].sort((a,b)=>{const aTotal=Math.max(Number(accountInTotals.get(a.key)?.amount||0),Number(accountOutTotals.get(a.key)?.amount||0));const bTotal=Math.max(Number(accountInTotals.get(b.key)?.amount||0),Number(accountOutTotals.get(b.key)?.amount||0));return bTotal-aTotal}).map((account,idx)=>{
    const realIn=Number(accountRealIncomeMap.get(account.key)||0);
    const out=Number(accountOutTotals.get(account.key)?.amount||0);
    return{id:`account:${account.key}`,key:account.key,label:account.label,icon:account.icon,total:Math.max(Number(accountInTotals.get(account.key)?.amount||0),out),color:MONEY_FLOW_PALETTES.account[idx%MONEY_FLOW_PALETTES.account.length],netChange:realIn-out,realIn,outflow:out};
  });
  let spendingColorIdx=0;
  let savingsColorIdx=0;
  const categories=[...groupedCategories.values()].sort((a,b)=>{const groupOrder={spending:0,savings:1,other:2};const ag=groupOrder[a.group]??99;const bg=groupOrder[b.group]??99;if(ag!==bg)return ag-bg;return b.amount-a.amount}).map(category=>{
    let color;
    if(category.label==='Other')color='#94a3b8';
    else if(category.group==='savings')color=MONEY_FLOW_PALETTES.savings[(savingsColorIdx++)%MONEY_FLOW_PALETTES.savings.length];
    else color=MONEY_FLOW_PALETTES.spending[(spendingColorIdx++)%MONEY_FLOW_PALETTES.spending.length];
    const stat=groupedCategoryStats.get(category.label)||{count:0,topNote:'',entries:[]};
    return{id:`category:${category.label}`,label:category.label,icon:category.icon,group:category.group,total:Number(category.amount||0),color,txCount:Number(stat.count||0),topNote:String(stat.topNote||''),topEntries:Array.isArray(stat.entries)?stat.entries.slice(0,5):[]};
  });
  const accountKeySet=new Set(accounts.map(account=>account.key));
  const monthTransfers=Array.isArray(transfers)?transfers.filter(t=>(t&&(t.date||'').slice(0,7)===monthKey&&Number(t.amount||0)>0&&t.from&&t.to&&!isExternalTransfer(t)&&accountKeySet.has(t.from)&&accountKeySet.has(t.to)&&t.from!==t.to)):[];
  const transferLinks=monthTransfers.map(t=>({id:`transfer:${t.id}`,source:`account:${t.from}`,target:`account:${t.to}`,value:Number(t.amount||0),fee:Number(t.fee||0),note:String(t.note||'').trim()}));
  const transferTotal=transferLinks.reduce((sum,link)=>sum+Number(link.value||0),0);
  ...
  const totalIncomeWithBalance=Number(monthIncomes.reduce((sum,income)=>sum+Number(income.amount||0),0))+Number(sourceTotals.get('Existing Balance')?.amount||0);
  const totalOutflowAll=Number(monthEntries.reduce((sum,entry)=>sum+Number(entry.amount||0),0));
  const computeAvgDay=ds=>(ds&&ds.weight>0)?Math.max(1,Math.min(31,Math.round(ds.sum/ds.weight))):null;
  const links=[
    ...[...sourceAccountFlows.values()].sort((a,b)=>(sourceTotals.get(b.source)?.amount||0)-(sourceTotals.get(a.source)?.amount||0)||((accountRank.get(a.account)??0)-(accountRank.get(b.account)??0))).map(flow=>{
      const value=Number(flow.amount||0);
      const dayKey=`${flow.source}|||${flow.account}`;
      const avgDay=computeAvgDay(sourceAccountDates.get(dayKey));
      return{id:`source:${flow.source}->account:${flow.account}`,source:`source:${flow.source}`,target:`account:${flow.account}`,value,color:rgbaFromHex(sourceColorByLabel.get(flow.source),.42),baseColor:sourceColorByLabel.get(flow.source)||'#94a3b8',pct:totalIncomeWithBalance>0?Math.round((value/totalIncomeWithBalance)*100):0,avgDay,stage:'income'};
    }),
    ...[...accountCategoryFlows.values()].sort((a,b)=>(accountRank.get(a.account)??0)-(accountRank.get(b.account)??0)||((categoryRank.get(a.label)??0)-(categoryRank.get(b.label)??0))).map(flow=>{
      const value=Number(flow.amount||0);
      const dayKey=`${flow.account}|||${flow.label}`;
      const avgDay=computeAvgDay(accountCategoryDates.get(dayKey));
      return{id:`account:${flow.account}->category:${flow.label}`,source:`account:${flow.account}`,target:`category:${flow.label}`,value,color:rgbaFromHex(categoryColorByLabel.get(flow.label),.40),baseColor:categoryColorByLabel.get(flow.label)||'#94a3b8',pct:totalOutflowAll>0?Math.round((value/totalOutflowAll)*100):0,avgDay,stage:'outflow'};
    })
  ];
...
function getMoneyFlowDeltaMarkup(curr,prev,direction){
  const c=Number(curr||0);
  const p=Number(prev||0);
  if(p===0&&c===0)return'';
  if(p===0)return`<span class="money-flow-sub money-flow-sub-muted" title="No data for last month">new</span>`;
  const diff=c-p;
  if(diff===0)return`<span class="money-flow-sub money-flow-sub-muted" title="Same as last month">—</span>`;
  const goingUp=diff>0;
  const goodDirection=direction==='lower-good'?!goingUp:goingUp;
  const cls=goodDirection?'money-flow-sub-good':'money-flow-sub-bad';
  const arrow=goingUp?'▲':'▼';
  return`<span class="money-flow-sub ${cls}" title="vs last month">${arrow} ${fmtShort(Math.abs(diff))}</span>`;
}
function getMoneyFlowSummaryMarkup(data,prevData){
  if(!data||!data.hasActivity)return'';
  const prev=prevData&&prevData.hasActivity?prevData:null;
  const spendingOutflow=Math.max(Number(data.totalOutflow||0)-Number(data.savingsOutflow||0),0);
  const prevSpendingOutflow=prev?Math.max(Number(prev.totalOutflow||0)-Number(prev.savingsOutflow||0),0):0;
  const netFlow=Number(data.totalIncome||0)-Number(data.totalOutflow||0);
  const netSign=netFlow<0?'−':netFlow>0?'+':'';
  const netToneClass=netFlow<0?'is-deficit':netFlow>0?'is-surplus':'is-zero';
  const spendingDelta=getMoneyFlowDeltaMarkup(spendingOutflow,prevSpendingOutflow,'lower-good');
  const overspending=spendingOutflow>Number(data.totalIncome||0);
  const incomeBase=Math.max(Number(data.totalIncome||0),1);
  const pacePct=Math.round((spendingOutflow/incomeBase)*100);
  const paceClampedFill=Math.min(100,pacePct);
  const paceOverFill=Math.max(0,Math.min(100,pacePct-100));
  const paceText=Number(data.totalIncome||0)<=0?'No income tracked yet'
    :overspending?`${pacePct}% of income · ${fmtShort(spendingOutflow-Number(data.totalIncome||0))} over`
    :pacePct>=85?`${pacePct}% of income · near limit`
    :`${pacePct}% of income`;
  const paceFillClass=overspending?'is-over':pacePct>=85?'is-warn':'is-safe';
  return`<div class="mf-stats-panel">
    <div class="mf-stats-hero">
      <div class="mf-stats-hero-row">
        <span class="mf-stats-hero-label">Spending this month</span>
        ${spendingDelta?`<span class="mf-stats-hero-delta">${spendingDelta}</span>`:''}
      </div>
      <div class="mf-stats-hero-amount ${overspending?'is-over':''}">${fmt(Math.round(spendingOutflow))}</div>
      <div class="mf-stats-pace">
        <div class="mf-stats-pace-bar"><div class="mf-stats-pace-fill ${paceFillClass}" style="width:${paceClampedFill}%"></div>${paceOverFill>0?`<div class="mf-stats-pace-over" style="width:${paceOverFill}%"></div>`:''}</div>
        <span class="mf-stats-pace-text">${esc(paceText)}</span>
      </div>
    </div>
    <div class="mf-stats-row">
      <div class="mf-stats-cell">
        <span class="mf-stats-cell-label">Income</span>
        <span class="mf-stats-cell-val is-income">${fmtShort(data.totalIncome)}</span>
      </div>
      <div class="mf-stats-cell">
        <span class="mf-stats-cell-label">Savings</span>
        <span class="mf-stats-cell-val is-savings">${fmtShort(data.savingsOutflow)}</span>
      </div>
      <div class="mf-stats-cell">
        <span class="mf-stats-cell-label">Net</span>
        <span class="mf-stats-cell-val ${netToneClass}">${netSign}${fmtShort(Math.abs(netFlow))}</span>
      </div>
    </div>
  </div>`;
}
function getMoneyFlowInsight(data){
  ...
  const netFlow=Number(data.totalIncome||0)-total;
  if(netFlow<0)return `You spent ${fmtShort(Math.abs(netFlow))} more than you earned this month. Tap the chart to see where it went.`;
...
function getMoneyFlowCaption(data){
  const bits=[];
  if(data&&data.accountsUsed)bits.push(`${data.accountsUsed} account${data.accountsUsed===1?'':'s'}`);
  if(data&&data.existingBalanceUsed>0)bits.push(`${fmtShort(data.existingBalanceUsed)} from existing balance`);
  return bits.join(' · ');
}
...
function getMoneyFlowFilterContext(data,filterMode=moneyFlowViewerFilter){
  ...
  if(filterMode==='transfers'){
    const transferAccountIds=new Set();
    (data?.transferLinks||[]).forEach(t=>{transferAccountIds.add(t.source);transferAccountIds.add(t.target);});
    transferAccountIds.forEach(id=>nodeIds.add(id));
    return{linkIds,nodeIds};
  }
...
function getMoneyFlowPeekDrawerMarkup(data,state){
  const breakdown=getMoneyFlowBreakdownItems(data,state);
  const peekLabel=(state.filterMode||'all')==='income'?'Where it came from':'Where it went';
  return`<div class="money-flow-detail-tray money-flow-side-drawer money-flow-side-drawer-peek" id="money-flow-detail-drawer" data-drawer-mode="peek">${getMoneyFlowDrawerHandleMarkup({label:peekLabel,meta:breakdown.length?String(breakdown.length):'0',ariaLabel:'Show money flow breakdown'})}</div>`;
}
function getMoneyFlowBreakdownItems(data,state){
  const filterMode=state.filterMode||'all';
  if(filterMode==='income'){
    return(data.sources||[]).filter(s=>state.visibleNodeIds.has(s.id)).sort((a,b)=>(b.total||0)-(a.total||0)).map(s=>({...s,kind:'source'}));
  }
  return(data.categories||[]).filter(c=>state.visibleNodeIds.has(c.id)).sort((a,b)=>(b.total||0)-(a.total||0)).map(c=>({...c,kind:'category'}));
}
function getMoneyFlowBreakdownTotal(data,state){
  const filterMode=state.filterMode||'all';
  if(filterMode==='income')return Number(data.totalIncome||0)+Number(data.existingBalanceUsed||0);
  if(filterMode==='savings')return Number(data.savingsOutflow||0);
  if(filterMode==='spending')return Math.max(Number(data.totalOutflow||0)-Number(data.savingsOutflow||0),0);
  return Number(data.totalOutflow||0);
}
function getMoneyFlowBreakdownRowMarkup(item,total,state){
  const value=Number(item.total||0);
  const pct=total>0?Math.round((value/total)*100):0;
  const trackPct=total>0?Math.min(100,(value/total)*100):0;
  const subBits=[];
  if(pct>0)subBits.push(`${pct}%`);
  if(item.kind==='category'&&Number(item.txCount||0)>0)subBits.push(`${item.txCount} tx`);
  if(item.topNote)subBits.push(esc(item.topNote.length>22?item.topNote.slice(0,21)+'…':item.topNote));
  const isSelected=state.selectedNodeIds.has(item.id);
  const groupClass=item.group?` money-flow-breakdown-row-${item.group}`:'';
  const safeId=encodeURIComponent(item.id);
  return`<button class="money-flow-breakdown-row${isSelected?' is-selected':''}${groupClass}" type="button" data-node-id="${safeId}" onclick="handleMoneyFlowNodeTap(decodeURIComponent('${safeId}'))"><span class="money-flow-breakdown-accent" style="background:${item.color}"></span><span class="money-flow-breakdown-main"><span class="money-flow-breakdown-label">${item.icon?`${esc(item.icon)} `:''}${esc(item.label)}</span><span class="money-flow-breakdown-track"><span class="money-flow-breakdown-track-fill" style="width:${trackPct.toFixed(1)}%;background:${item.color}"></span></span>${subBits.length?`<span class="money-flow-breakdown-sub">${subBits.join(' · ')}</span>`:''}</span><span class="money-flow-breakdown-amount">${fmtShort(value)}</span></button>`;
}
function getMoneyFlowDefaultTrayMarkup(data,state){
  const summary=getMoneyFlowDefaultTrayData(data,state);
  const filterMode=state.filterMode||'all';
  const breakdown=getMoneyFlowBreakdownItems(data,state);
  const breakdownTotal=getMoneyFlowBreakdownTotal(data,state);
  const isIncomeMode=filterMode==='income';
  const trayTitle=isIncomeMode?'Where it came from':'Where it went';
  const totalLabel=isIncomeMode?'Total in':filterMode==='savings'?'Saved':filterMode==='spending'?'Spent':'Total out';
  if(!summary.visibleLinks.length){
    return`<div class="money-flow-detail-tray money-flow-side-drawer money-flow-side-drawer-open money-flow-detail-tray-compact" id="money-flow-detail-drawer" data-drawer-mode="open">${getMoneyFlowDrawerHandleMarkup({expanded:true,label:trayTitle})}<div class="money-flow-side-drawer-panel"><div class="money-flow-detail-head"><div><div class="money-flow-detail-kicker">${esc(getMoneyFlowFilterLabel(summary.filterMode))}</div><div class="money-flow-detail-title">No matching routes</div></div></div><div class="money-flow-detail-hint">This filter does not have tracked flow in ${esc(summary.monthLabel)}. Try another filter or switch back to All.</div></div></div>`;
  }
  const breakdownRows=breakdown.length?breakdown.map(item=>getMoneyFlowBreakdownRowMarkup(item,breakdownTotal,state)).join(''):'<div class="money-flow-detail-hint">No items to break down for this filter.</div>';
  return`<div class="money-flow-detail-tray money-flow-side-drawer money-flow-side-drawer-open" id="money-flow-detail-drawer" data-drawer-mode="open">${getMoneyFlowDrawerHandleMarkup({expanded:true,label:trayTitle,meta:String(breakdown.length)})}<div class="money-flow-side-drawer-panel"><div class="money-flow-detail-head"><div><div class="money-flow-detail-kicker">${esc(summary.monthLabel)}</div><div class="money-flow-detail-title">${esc(trayTitle)}</div><div class="money-flow-detail-sub">${esc(totalLabel)}: <strong>${fmt(breakdownTotal)}</strong></div></div><div class="money-flow-detail-pill">${breakdown.length} item${breakdown.length===1?'':'s'}</div></div><div class="money-flow-breakdown-list">${breakdownRows}</div><div class="money-flow-detail-hint">Tap a row to highlight that ${isIncomeMode?'source':'category'} in the chart. Double tap empty space to reset.</div></div></div>`;
}
function getMoneyFlowBackButtonMarkup(){
  return`<button type="button" class="money-flow-detail-back" aria-label="Back to overview" title="Back to overview" onclick="clearMoneyFlowSelection()"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg></button>`;
}
function getMoneyFlowSelectionTrayMarkup(data,state){
  ...
  if(state.selection.type==='link'){
    ...
    return`<div class="money-flow-detail-tray money-flow-side-drawer money-flow-side-drawer-open is-selection" id="money-flow-detail-drawer" data-drawer-mode="open">${getMoneyFlowDrawerHandleMarkup({expanded:true})}<div class="money-flow-side-drawer-panel"><div class="money-flow-detail-head"><div class="money-flow-detail-head-titles">${getMoneyFlowBackButtonMarkup()}<div class="money-flow-detail-head-text"><div class="money-flow-detail-kicker">${routeType}</div><div class="money-flow-detail-title">${source?esc(source.label):'Source'} → ${target?esc(target.label):'Target'}</div></div></div><div class="money-flow-detail-pill">${fmtShort(link.value)}</div></div><div class="money-flow-detail-stats money-flow-detail-stats-row"><div class="money-flow-detail-stat"><span class="money-flow-detail-label">Value</span><strong>${fmtShort(link.value)}</strong></div><div class="money-flow-detail-stat"><span class="money-flow-detail-label">Share</span><strong>${getMoneyFlowPercentLabel(link.value,shareBase)}</strong></div><div class="money-flow-detail-stat"><span class="money-flow-detail-label">Stage</span><strong>${routeType.replace(' route','')}</strong></div></div></div></div>`;
  }
  ...
  if(kind==='source'){
    ...
    stats=[
      {label:'Total',value:fmtShort(node.total)},
      {label:'Share',value:getMoneyFlowPercentLabel(node.total,(data.totalIncome||0)+(data.existingBalanceUsed||0))},
      {label:'Top to',value:topOut?esc(topOut.label):'—'}
    ];
  }else if(kind==='account'){
    kicker='Account';
    stats=[
      {label:'In',value:fmtShort(incomingTotal)},
      {label:'Out',value:fmtShort(outgoingTotal)},
    ...
    stats=[
      {label:'Received',value:fmtShort(incomingTotal||node.total)},
  ...
  const isCategoryNode=kind==='category';
  const txMarkup=isCategoryNode?getMoneyFlowTopEntriesMarkup(node):'';
  const canOpenHistory=isCategoryNode&&node.label&&node.label!=='Other'&&node.label!=='Leftover';
  const historyBtn=canOpenHistory?`<button type="button" class="money-flow-history-btn" onclick="openMoneyFlowCategoryInHistory('${esc(String(node.label).replace(/'/g,"\\'"))}')">Open in History →</button>`:'';
  return`<div class="money-flow-detail-tray money-flow-side-drawer money-flow-side-drawer-open is-selection" id="money-flow-detail-drawer" data-drawer-mode="open">${getMoneyFlowDrawerHandleMarkup({expanded:true})}<div class="money-flow-side-drawer-panel"><div class="money-flow-detail-head"><div class="money-flow-detail-head-titles">${getMoneyFlowBackButtonMarkup()}<div class="money-flow-detail-head-text"><div class="money-flow-detail-kicker">${kicker}</div><div class="money-flow-detail-title">${esc(node.label)}</div></div></div><div class="money-flow-detail-pill">${connectedLinks.length} route${connectedLinks.length===1?'':'s'}</div></div><div class="money-flow-detail-stats money-flow-detail-stats-row">${stats.map(stat=>`<div class="money-flow-detail-stat"><span class="money-flow-detail-label">${stat.label}</span><strong>${stat.value}</strong></div>`).join('')}</div>${txMarkup}${historyBtn}</div></div>`;
}
...
function getMoneyFlowFilterChipsMarkup(){
  return`<div class="mf-fs-segmented" role="tablist">${MONEY_FLOW_FILTERS.map(filter=>`<button class="mf-fs-segment ${moneyFlowViewerFilter===filter.key?'is-active':''}" type="button" role="tab" aria-pressed="${moneyFlowViewerFilter===filter.key?'true':'false'}" onclick="setMoneyFlowFilter('${filter.key}')">${filter.label}</button>`).join('')}</div>`;
}
function openMoneyFlowCategoryInHistory(label){
  const target=String(label||'').trim();
  if(!target||target==='Other'||target==='Leftover')return;
  closeMoneyFlowFullscreen();
  try{
    if(typeof setHistorySelectedCategories==='function')setHistorySelectedCategories([target]);
    if(typeof setHistoryQuickPreset==='function')setHistoryQuickPreset('thisMonth');
    if(typeof showTab==='function')showTab('history');
    if(typeof render==='function')render();
    if(typeof showActionToast==='function')showActionToast('Filtered to category',target,'🔍');
  }catch(err){console&&console.warn&&console.warn('openMoneyFlowCategoryInHistory failed',err);}
}
function clearMoneyFlowSelection(){
  if(!moneyFlowViewerSelection)return;
  moneyFlowViewerSelection=null;
  moneyFlowDrawerMode='summary';
  moneyFlowLastBackgroundTapAt=0;
  rerenderMoneyFlowFullscreen(true);
}
function formatMoneyFlowDate(dateStr){
...
function stripCategoryPrefix(note,categoryLabel){
...
function getMoneyFlowTopEntriesMarkup(node){
  const list=Array.isArray(node?.topEntries)?node.topEntries:[];
  if(!list.length)return'';
  const categoryLabel=String(node?.label||'').trim();
  const rows=list.slice(0,5).map(entry=>{
    const cleanedNote=stripCategoryPrefix(entry.note,categoryLabel);
    const accountInfo=entry.account?(typeof getAccountInfo==='function'?getAccountInfo(entry.account):{name:''}):{name:''};
    const title=cleanedNote||accountInfo.name||categoryLabel||'Transaction';
    const dateNice=formatMoneyFlowDate(entry.date);
    const metaBits=[];
    if(dateNice)metaBits.push(dateNice);
    if(accountInfo.name&&cleanedNote)metaBits.push(accountInfo.name);
    return`<div class="money-flow-tx-row"><div class="money-flow-tx-meta"><span class="money-flow-tx-note">${esc(title)}</span>${metaBits.length?`<span class="money-flow-tx-date">${esc(metaBits.join(' · '))}</span>`:''}</div><span class="money-flow-tx-amount">${fmt(Number(entry.amount||0))}</span></div>`;
  }).join('');
  return`<div class="money-flow-tx-list"><div class="money-flow-tx-head">Top transactions</div>${rows}</div>`;
}
function makeMoneyFlowSvg(data,opts={}){
  ...
  const svgIdSafe=(opts.svgId||'mf').replace(/[^a-z0-9]/gi,'');
  const linkGeom=data.links.map(link=>{
    ...
    const gradId=`${svgIdSafe}_g_${link.id.replace(/[^a-z0-9]/gi,'_')}`;
    return{link,thickness,x1,y1,x2,y2,midX:(x1+x2)/2,midY:(y1+y2)/2,gradId};
  });
  const gradientDefs=linkGeom.filter(Boolean).map(g=>{
    const day=Number(g.link.avgDay||15);
    const endAlpha=0.20+Math.min(31,Math.max(1,day))/31*0.42;
    const base=g.link.baseColor||'#94a3b8';
    return`<linearGradient id="${g.gradId}" x1="${g.x1}" y1="${g.y1}" x2="${g.x2}" y2="${g.y2}" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-color="${rgbaFromHex(base,0.12)}"/><stop offset="100%" stop-color="${rgbaFromHex(base,endAlpha)}"/></linearGradient>`;
  }).join('');
  const paths=linkGeom.filter(Boolean).map(g=>{
    const curve=Math.max((g.x2-g.x1)*.38,48);
    const linkClasses=['money-flow-link'];
    if(interactive){
      linkClasses.push('is-interactive');
      if(focusMode){
        if(viewState?.selectedLinkIds.has(g.link.id))linkClasses.push('is-selected');
        else if(viewState?.activeLinkIds.has(g.link.id))linkClasses.push('is-active');
        else linkClasses.push('is-muted');
      }
    }
    const dataAttr=interactive?` data-link-id="${encodeURIComponent(g.link.id)}"`:'';
    return`<path class="${linkClasses.join(' ')}"${dataAttr} d="M ${g.x1} ${g.y1} C ${g.x1+curve} ${g.y1}, ${g.x2-curve} ${g.y2}, ${g.x2} ${g.y2}" stroke="url(#${g.gradId})" stroke-width="${g.thickness}"></path>`;
  }).join('');
  const linkLabels=linkGeom.filter(g=>g&&Number(g.link.pct||0)>=5&&g.thickness>=14).map(g=>{
    const isFocused=interactive&&focusMode&&!(viewState?.activeLinkIds.has(g.link.id));
    if(isFocused)return'';
    const w=g.link.pct>=10?34:28;
    return`<g class="money-flow-link-pct" pointer-events="none"><rect x="${g.midX-w/2}" y="${g.midY-9}" width="${w}" height="18" rx="6"></rect><text x="${g.midX}" y="${g.midY+4}" text-anchor="middle">${g.link.pct}%</text></g>`;
  }).join('');
  ...
  const nodes=[...data.sources,...data.accounts,...data.categories].map(node=>{
    const isAccountNode=node.id.startsWith('account:');
    ...
    let netLine='';
    if(isAccountNode&&Number.isFinite(node.netChange)&&node.height>=subLabelY+22){
      const positive=Number(node.netChange)>=0;
      const sign=positive?'+':'−';
      netLine=`<text class="money-flow-node-net ${positive?'is-pos':'is-neg'}" x="${node.x+labelX}" y="${node.y+subLabelY+14}">${sign}${esc(fmtShort(Math.abs(node.netChange)))} net</text>`;
    }
    ...
    return`<g class="${nodeClasses.join(' ')}"${dataAttr}><rect class="money-flow-node" x="${node.x}" y="${node.y}" width="${nodeWidth}" height="${node.height}" rx="${nodeRadius}"></rect><rect class="money-flow-node-tint" x="${node.x}" y="${node.y}" width="${nodeWidth}" height="${node.height}" rx="${nodeRadius}" fill="${node.color}" fill-opacity="0.07"></rect><rect class="money-flow-node-accent" x="${node.x}" y="${node.y}" width="${accentWidth}" height="${node.height}" rx="${nodeRadius}" fill="${node.color}"></rect><text class="money-flow-node-label" x="${node.x+labelX}" y="${node.y+labelY}">${esc(label)}</text>${amountLine}${netLine}</g>`;
  }).join('');
  const transferLinks=Array.isArray(data.transferLinks)?data.transferLinks:[];
  const showTransfers=interactive?(opts.filterMode||moneyFlowViewerFilter)!=='income':true;
  const transferPaths=showTransfers?transferLinks.map(t=>{
    const source=nodeById.get(t.source);
    const target=nodeById.get(t.target);
    if(!source||!target||source===target)return null;
    const sx=source.x+nodeWidth-2;
    const sy=source.y+source.height/2;
    const tx=target.x+nodeWidth-2;
    const ty=target.y+target.height/2;
    const offset=Math.max(38,Math.abs(sy-ty)*.25+30);
    const cx=Math.max(sx,tx)+offset;
    const midX=(sx+cx+cx+tx)/4;
    const midY=(sy+ty)/2;
    const muted=interactive&&focusMode;
    return{sx,sy,tx,ty,cx,midX,midY,value:Number(t.value||0),id:t.id,muted};
  }).filter(Boolean):[];
  const transferLayer=transferPaths.length?`<g class="money-flow-transfers" pointer-events="none">${transferPaths.map(p=>`<path class="money-flow-transfer-arc${p.muted?' is-muted':''}" d="M ${p.sx} ${p.sy} C ${p.cx} ${p.sy}, ${p.cx} ${p.ty}, ${p.tx} ${p.ty}"></path>${p.value>=1?`<g class="money-flow-transfer-label"><rect x="${p.midX-22}" y="${p.midY-9}" width="44" height="18" rx="6"></rect><text x="${p.midX}" y="${p.midY+4}" text-anchor="middle">↔ ${esc(fmtShort(p.value))}</text></g>`:''}`).join('')}</g>`:'';
  const clipDef=linkClipRects.length?`<clipPath id="${linkClipId}" clipPathUnits="userSpaceOnUse">${linkClipRects.map(rect=>`<rect x="${rect.x}" y="${rect.y}" width="${rect.width}" height="${rect.height}"></rect>`).join('')}</clipPath>`:'';
  const defs=`<defs>${clipDef}${gradientDefs}</defs>`;
  const linkLayer=linkClipRects.length?`<g clip-path="url(#${linkClipId})">${paths}</g>`:`<g>${paths}</g>`;
  const linkLabelLayer=linkClipRects.length?`<g clip-path="url(#${linkClipId})">${linkLabels}</g>`:`<g>${linkLabels}</g>`;
  return`<svg${svgIdAttr} class="${svgClass}" data-base-width="${width}" data-base-height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(ariaLabel)}">${defs}<g>${stageLabels.map(stage=>`<text class="money-flow-stage" x="${stage.x}" y="${stageLabelY}" text-anchor="middle">${stage.label}</text><line class="money-flow-stage-line" x1="${stage.x-28}" y1="${stageLabelY+7}" x2="${stage.x+28}" y2="${stageLabelY+7}" stroke-linecap="round"></line>`).join('')}</g>${linkLayer}${linkLabelLayer}<g>${nodes}</g>${transferLayer}</svg>`;
}
...
function renderMoneyFlowFullscreenContent(data=buildMoneyFlowData(),opts={}){
  ...
  const detailTrayMarkup=getMoneyFlowDetailTrayMarkup(data,viewState);
  const filterChipsMarkup=getMoneyFlowFilterChipsMarkup();
  const monthLabel=getMoneyFlowMonthLabel(data.monthKey||currentMonthKey());
  const spendingOutflow=Math.max(Number(data.totalOutflow||0)-Number(data.savingsOutflow||0),0);
  mount.innerHTML=`<div class="money-flow-fullscreen-viewer mf-fs-viewer"><div class="money-flow-fullscreen-overlay mf-fs-overlay"><div class="money-flow-fullscreen-topbar mf-fs-topbar"><div class="mf-fs-topbar-left"><div class="mf-fs-month">${esc(monthLabel)}</div><div class="mf-fs-meta"><span class="mf-fs-meta-dot" aria-hidden="true"></span>${fmtShort(spendingOutflow)} spent</div></div><div class="mf-fs-topbar-center money-flow-fullscreen-controls">${filterChipsMarkup}</div><div class="mf-fs-topbar-right"><div class="mf-fs-zoom-cluster"><button class="mf-fs-icon-btn" id="money-flow-zoom-out" type="button" aria-label="Zoom out" title="Zoom out" onclick="stepMoneyFlowZoom(-1)">−</button><button class="mf-fs-icon-btn mf-fs-zoom-reset" id="money-flow-zoom-reset" type="button" aria-label="Reset zoom" title="Reset zoom" onclick="resetMoneyFlowZoom()">100%</button><button class="mf-fs-icon-btn" id="money-flow-zoom-in" type="button" aria-label="Zoom in" title="Zoom in" onclick="stepMoneyFlowZoom(1)">+</button></div><span class="mf-fs-divider"></span><div class="mf-fs-theme-cluster"><button class="mf-fs-icon-btn money-flow-theme-btn" id="money-flow-theme-light" type="button" aria-label="Use light mode" title="Light mode" onclick="setMoneyFlowViewerTheme('light')"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 3v1M12 20v1M3 12h1M20 12h1M5.6 5.6l.7.7M17.7 17.7l.7.7M5.6 18.4l.7-.7M17.7 6.3l.7-.7"/></svg></button><button class="mf-fs-icon-btn money-flow-theme-btn" id="money-flow-theme-dark" type="button" aria-label="Use dark mode" title="Dark mode" onclick="setMoneyFlowViewerTheme('dark')"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 13a8.5 8.5 0 1 1-10-10 6.5 6.5 0 0 0 10 10z"/></svg></button></div><span class="mf-fs-divider"></span><button class="mf-fs-icon-btn mf-fs-close" type="button" aria-label="Close fullscreen money flow" title="Close" onclick="closeMoneyFlowFullscreen()"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6l12 12M18 6L6 18"/></svg></button></div></div></div><div class="money-flow-fullscreen-stage mf-fs-stage" id="money-flow-fullscreen-stage"><div class="money-flow-fullscreen-canvas mf-fs-canvas">${makeMoneyFlowSvg(data,{width:1600,height:900,nodeWidth:238,marginTop:64,marginBottom:24,gapY:22,minNodeHeight:26,labelMaxCategory:26,labelMaxDefault:24,stageLabelY:26,labelY:21,subLabelY:40,svgClass:'money-flow-svg money-flow-fullscreen-svg',svgId:'money-flow-fullscreen-svg',ariaLabel:'Fullscreen money flow Sankey diagram for the current month',interactive:true,filterMode:moneyFlowViewerFilter,selection:moneyFlowViewerSelection})}</div></div>${detailTrayMarkup}</div>`;
  applyMoneyFlowZoom(moneyFlowZoom);
  applyMoneyFlowViewerTheme();
  setupMoneyFlowStageGestures();
  setupMoneyFlowInteractiveHandlers();
  setupMoneyFlowDrawerGestures();
  const stage=document.getElementById('money-flow-fullscreen-stage');
...
function renderMoneyFlowCard(){
  ...
  const caption=getMoneyFlowCaption(data);
  const prevData=buildMoneyFlowData(getPreviousMonthKey(data.monthKey||currentMonthKey()));
  mount.innerHTML=`<div class="card money-flow-card-shell"><div class="card-header"><span class="card-title">Money Flow</span>${headerActions}</div>${helpMode?'<div class="help-inline">Tracks this month\'s flow from income sources into accounts, then out into your biggest spending and savings categories.</div>':''}<div class="money-flow-summary">${getMoneyFlowSummaryMarkup(data,prevData)}</div><div id="money-flow-chart" class="money-flow-chart money-flow-chart-preview" onclick="openMoneyFlowFullscreen()" title="Open fullscreen money flow view">${makeMoneyFlowSvg(data)}</div><div class="money-flow-caption">${caption?`<span>${caption}</span>`:''}<span class="money-flow-inline-link">Tap to explore →</span></div></div>`;
  syncMoneyFlowFullscreen(data);
}
...
function getNetWorthSummary(){
  const assetAccounts=nwAccounts.filter(a=>a.accountType!=='credit_card');
  const creditCardAccounts=nwAccounts.filter(a=>a.accountType==='credit_card');
  const assetTotal=roundMoney(assetAccounts.reduce((sum,account)=>sum+Number(nwBalances[account.key]||0),0));
  const creditCardTotal=roundMoney(creditCardAccounts.reduce((sum,account)=>sum+Number(nwBalances[account.key]||0),0));
  const linkedDebtIds=new Set(creditCardAccounts.filter(account=>account.linkedDebtId).map(account=>String(account.linkedDebtId)));
  const unlinkedDebtTotal=roundMoney(debts.reduce((sum,debt)=>linkedDebtIds.has(String(debt.id))?sum:sum+Number(debt.total||0),0));
  const debtTotal=roundMoney(creditCardTotal+unlinkedDebtTotal);
  return{assetAccounts,creditCardAccounts,assetTotal,creditCardTotal,unlinkedDebtTotal,debtTotal,net:roundMoney(assetTotal-debtTotal)};
}
function getTrackedAssetBalance(){return getNetWorthSummary().assetTotal}
function syncNwAccountBalanceField(){
  const balanceRow=document.getElementById('nw-acc-balance-row');
  const balanceInput=document.getElementById('nw-acc-balance');
  const balanceLabel=balanceRow?.querySelector('.input-label');
  const linkedDebtSelect=document.getElementById('nw-linked-debt');
  if(!balanceRow||!balanceInput||!balanceLabel)return;
  const isCreditCard=document.getElementById('nw-type-credit')?.classList.contains('active');
  const linkedDebtId=isCreditCard?(linkedDebtSelect?.value||''):'';
  if(!isCreditCard){
    balanceLabel.textContent='Opening Balance (₱)';
    balanceInput.placeholder='0';
    balanceRow.style.display='';
    renderNwModalPreview();
    return;
  }
  balanceLabel.textContent='Current Balance Owed (₱)';
  balanceInput.placeholder='e.g. 4800';
  if(linkedDebtId){
    const linkedDebt=debts.find(d=>String(d.id)===String(linkedDebtId));
    if(linkedDebt)balanceInput.value=moneyInputValue(Math.max(0,Number(linkedDebt.total||0)));
...
const NW_BANK_PRESETS=[
  {name:'GCash',icon:'📱',type:'asset'},{name:'Maya',icon:'📱',type:'asset'},
  {name:'BPI Savings',icon:'🏦',type:'asset'},{name:'BPI Credit',icon:'💳',type:'credit_card'},
  {name:'BDO Savings',icon:'🏦',type:'asset'},{name:'BDO Credit',icon:'💳',type:'credit_card'},
  {name:'Metrobank',icon:'🏦',type:'asset'},{name:'RCBC Savings',icon:'🏦',type:'asset'},
  {name:'RCBC Credit',icon:'💳',type:'credit_card'},{name:'UnionBank',icon:'🏦',type:'asset'},
  {name:'Security Bank',icon:'🏦',type:'asset'},{name:'LandBank',icon:'🏦',type:'asset'},
  {name:'PNB',icon:'🏦',type:'asset'},{name:'ChinaBank',icon:'🏦',type:'asset'},
  {name:'EastWest',icon:'🏦',type:'asset'},{name:'Wise USD',icon:'💵',type:'asset'},
  {name:'Cash',icon:'💵',type:'asset'},{name:'Bitcoin',icon:'🪙',type:'asset'},
  {name:'XM',icon:'📈',type:'asset'},
];
function applyNwPreset(el,name,icon,type){document.getElementById('nw-acc-name').value=name;document.getElementById('nw-acc-icon').value=icon;setNwAccountType(type);document.querySelectorAll('.nw-preset-chip').forEach(c=>c.classList.remove('active'));el.classList.add('active');(type==='credit_card'?document.getElementById('nw-cc-limit'):document.getElementById('nw-acc-balance')).focus();}
function _makeDragScroll(el){if(!el||el._dragBound)return;el._dragBound=true;let down=false,startX,sl;el.addEventListener('mousedown',e=>{down=true;startX=e.pageX-el.offsetLeft;sl=el.scrollLeft;el.style.cursor='grabbing';el.style.userSelect='none'});el.addEventListener('mouseleave',()=>{down=false;el.style.cursor='';el.style.userSelect=''});el.addEventListener('mouseup',()=>{down=false;el.style.cursor='';el.style.userSelect=''});el.addEventListener('mousemove',e=>{if(!down)return;e.preventDefault();el.scrollLeft=sl-(e.pageX-el.offsetLeft-startX)*1.2});}
function openNetWorthAdd(){editingNetWorthKey=null;document.getElementById('nw-modal-title').textContent='Add Account';const kicker=document.getElementById('nw-modal-kicker');if(kicker)kicker.textContent='Create Net Worth Account';document.getElementById('nw-acc-icon').value='🏦';document.getElementById('nw-acc-name').value='';document.getElementById('nw-acc-balance').value='';document.getElementById('nw-cc-limit').value='';document.getElementById('nw-delete-btn').style.display='none';document.getElementById('nw-transfer-btn').style.display='none';setNwAccountType('asset');populateNwDebtSelect();const pw=document.getElementById('nw-bank-presets');if(pw){const usedNames=new Set(nwAccounts.map(a=>(a.name||'').trim().toLowerCase()));const available=NW_BANK_PRESETS.filter(p=>!usedNames.has(p.name.toLowerCase()));pw.style.display=available.length?'':'none';pw.innerHTML=available.map(p=>{const brand=getBankBrandVisual(p.name,p.icon);return`<button type="button" class="nw-preset-chip" style="--chip-bg:${brand.bg}" onclick="applyNwPreset(this,'${p.name}','${p.icon}','${p.type}')"><span class="nw-preset-logo">${nwBrandLogoHtml(brand,p.icon)}</span><span class="nw-preset-label">${p.name}</span></button>`;}).join('');_makeDragScroll(pw);}renderNwModalPreview();openModal('modal-nw-account')}
function openNetWorthEdit(key){const acc=nwAccounts.find(a=>a.key===key);if(!acc)return;editingNetWorthKey=key;const pw=document.getElementById('nw-bank-presets');if(pw)pw.style.display='none';document.getElementById('nw-modal-title').textContent='Edit Account';const kicker=document.getElementById('nw-modal-kicker');if(kicker)kicker.textContent='Account Settings';document.getElementById('nw-acc-icon').value=acc.icon||'🏦';document.getElementById('nw-acc-name').value=acc.name||'';document.getElementById('nw-acc-balance').value=moneyInputValue(nwBalances[key]||0);document.getElementById('nw-cc-limit').value=acc.creditLimit?moneyInputValue(acc.creditLimit):'';document.getElementById('nw-delete-btn').style.display='inline-flex';setNwAccountType(acc.accountType||'asset');populateNwDebtSelect(acc.linkedDebtId||'');if(acc.accountType!=='credit_card')document.getElementById('nw-transfer-btn').style.display='inline-flex';renderNwModalPreview();openModal('modal-nw-account')}
function saveNetWorthAccount(){
  ...
  const creditLimit=isCreditCard?roundMoney(parseFloat(document.getElementById('nw-cc-limit').value)||0):0;
  const linkedDebt=linkedDebtId?debts.find(d=>String(d.id)===String(linkedDebtId)):null;
  const balance=roundMoney(isCreditCard&&linkedDebt?Number(linkedDebt.total||0):(parseFloat(document.getElementById('nw-acc-balance').value)||0));
  if(!name){showAlert('Enter an account name.');return;}
  if(nwAccounts.some(a=>a.key!==editingNetWorthKey&&String(a.name||'').trim().toLowerCase()===name.toLowerCase())){showAlert('That account name already exists.');return;}
...
function adjustAccountBalance(key,delta){if(!key)return;if(nwBalances[key]===undefined)nwBalances[key]=0;nwBalances[key]=roundMoney(Number(nwBalances[key]||0)+Number(delta||0))}
...
function getBankBrandVisual(name,fallbackLogo=''){const n=(name||'').toLowerCase();if(n.includes('gcash')||n.includes('gcredit'))return{bg:'#0070ba',logo:'G',domain:'gcash.com'};if(n.includes('bpi'))return{bg:'#c8102e',logo:'BPI',domain:'bpi.com.ph'};if(n.includes('bdo'))return{bg:'#0a2e6e',logo:'BDO',domain:'bdo.com.ph'};if(n.includes('maya')||n.includes('paymaya'))return{bg:'#01a14b',logo:'M',domain:'maya.ph'};if(n.includes('metrobank')||n.includes('metro bank'))return{bg:'#c9a227',logo:'MB',domain:'metrobank.com.ph'};if(n.includes('rcbc'))return{bg:'#e8761a',logo:'RCBC',domain:'rcbc.com'};if(n.includes('wise'))return{bg:'#163300',logo:'W',domain:'wise.com'};if(n.includes('security bank'))return{bg:'#a00c20',logo:'SB',domain:'securitybank.com.ph'};if(n.includes('unionbank')||n.includes('union bank'))return{bg:'#cf202e',logo:'UB',domain:'unionbankph.com'};if(n.includes('landbank')||n.includes('land bank'))return{bg:'#165a16',logo:'LB',domain:'landbank.com'};if(n.includes('pnb'))return{bg:'#003087',logo:'PNB',domain:'pnb.com.ph'};if(n.includes('chinabank')||n.includes('china bank'))return{bg:'#8b0000',logo:'CB',domain:'chinabank.ph'};if(n.includes('eastwest'))return{bg:'#1a237e',logo:'EW',domain:'eastwestbanker.com'};if(n.includes('bitcoin')||n.includes('btc'))return{bg:'#f7931a',logo:'₿',domain:'bitcoin.org'};if(n.includes('xm'))return{bg:'#c8341c',logo:'XM',domain:'xm.com'};if(n.includes('cash'))return{bg:'#16a34a',logo:'₱'};if(n.includes('usd')||n.includes('dollar'))return{bg:'#1d6fa4',logo:'$'};if(n.includes('savings'))return{bg:'#6d28d9',logo:'S'};const fallback=(fallbackLogo||'').trim();return{bg:'#4f46e5',logo:fallback||((name||'?').trim()[0]||'?').toUpperCase()};}
function nwBrandLogoHtml(brand,fallbackLogo=''){const fallback=esc((fallbackLogo||brand.logo||'?')+'');if(brand?.domain){const src=`https://www.google.com/s2/favicons?sz=128&domain=${encodeURIComponent(brand.domain)}`;return`<img src="${src}" class="nw-logo-img" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><span class="nw-logo-letter" style="display:none">${fallback}</span>`;}return`<span class="nw-logo-letter">${fallback}</span>`;}
function renderNwModalPreview(){const mount=document.getElementById('nw-modal-preview');if(!mount)return;const icon=document.getElementById('nw-acc-icon')?.value||'🏦';const isCreditCard=document.getElementById('nw-type-credit')?.classList.contains('active');const rawName=(document.getElementById('nw-acc-name')?.value||'').trim();const linkedDebtId=isCreditCard?(document.getElementById('nw-linked-debt')?.value||''):'';const creditLimit=roundMoney(isCreditCard?(parseFloat(document.getElementById('nw-cc-limit')?.value)||0):0);const linkedDebt=linkedDebtId?debts.find(d=>String(d.id)===String(linkedDebtId)):null;const balance=roundMoney(isCreditCard&&linkedDebt?Number(linkedDebt.total||0):(parseFloat(document.getElementById('nw-acc-balance')?.value)||0));const brand=getBankBrandVisual(rawName,icon);
// Material variant
let materialClass='';let materialLabel='Classic';
if(isCreditCard){if(creditLimit>=200000){materialClass='nw-card-platinum';materialLabel='Platinum';}else if(creditLimit>=50000){materialClass='nw-card-gold';materialLabel='Gold';}}
else{if(balance>=500000){materialClass='nw-card-platinum';materialLabel='Platinum';}else if(balance>=100000){materialClass='nw-card-gold';materialLabel='Gold';}}
// Network badge
const _nn=(rawName||'').toLowerCase();let networkBadge='';
if(_nn.includes('visa'))networkBadge=`<div class="nw-card-network"><svg width="50" height="17" viewBox="0 0 50 17"><text x="0" y="14" font-family="Arial Black,Arial,sans-serif" font-size="15" font-weight="900" font-style="italic" fill="rgba(255,255,255,.92)" letter-spacing="1">VISA</text></svg></div>`;
else if(_nn.includes('mastercard'))networkBadge=`<div class="nw-card-network"><svg width="44" height="28" viewBox="0 0 44 28"><circle cx="16" cy="14" r="13" fill="#eb001b" opacity=".88"/><circle cx="28" cy="14" r="13" fill="#f79e1b" opacity=".88"/></svg></div>`;
// Utilization
const usedPct=creditLimit>0?Math.min(balance/creditLimit*100,100):0;const available=creditLimit>0?Math.max(creditLimit-balance,0):null;
let utilColor='rgba(255,255,255,.78)';if(usedPct>=80)utilColor='#fca5a5';else if(usedPct>=50)utilColor='#fde68a';else if(usedPct>0)utilColor='#86efac';
// Monthly change
const _now=new Date();const _ym=`${_now.getFullYear()}-${String(_now.getMonth()+1).padStart(2,'0')}`;let changeHtml='';const _ek=editingNetWorthKey;
if(_ek){if(isCreditCard){const _sp=entries.filter(e=>e.account===_ek&&(e.date||'').startsWith(_ym)).reduce((s,e)=>s+Number(e.amount||0),0);const _pd=debtPayments.filter(p=>p.account===_ek&&(p.date||'').startsWith(_ym)).reduce((s,p)=>s+Number(p.amount||0),0);const _pts=[];if(_sp>0)_pts.push(`<span class="nw-card-change-item nw-card-change-neg">↑ ${fmtShort(_sp)} charged this mo.</span>`);if(_pd>0)_pts.push(`<span class="nw-card-change-item nw-card-change-pos">↓ ${fmtShort(_pd)} paid this mo.</span>`);if(_pts.length)changeHtml=`<div class="nw-card-change">${_pts.join('')}</div>`;}else{const _ti=incomes.filter(i=>i.account===_ek&&(i.date||'').startsWith(_ym)).reduce((s,i)=>s+Number(i.amount||0),0);const _to=entries.filter(e=>e.account===_ek&&(e.date||'').startsWith(_ym)).reduce((s,e)=>s+Number(e.amount||0),0);const _net=_ti-_to;if(_ti>0||_to>0){const _cls=_net>=0?'nw-card-change-pos':'nw-card-change-neg';changeHtml=`<div class="nw-card-change"><span class="nw-card-change-item ${_cls}">${_net>=0?'↑':'↓'} ${fmtShort(Math.abs(_net))} this month</span></div>`;}}}
const metaLine=!isCreditCard?(rawName?'Included in assets and net worth':'Set a starting balance for this account'):'';
const chips=[isCreditCard&&linkedDebt?`<span class="nw-modal-preview-chip">Linked to ${esc(linkedDebt.name)}</span>`:'',isCreditCard&&!linkedDebt?'<span class="nw-modal-preview-chip">Manual balance</span>':'',creditLimit>0?`<span class="nw-modal-preview-chip">Limit ${fmtShort(creditLimit)}</span>`:'',materialLabel!=='Classic'?`<span class="nw-modal-preview-chip">${materialLabel}</span>`:'',!isCreditCard&&balance>0?'<span class="nw-modal-preview-chip">Ready to track</span>':''].filter(Boolean).join('');
const chipEl=isCreditCard?'<div class="nw-card-chip"></div>':'';
const balanceLabelEl=isCreditCard?'<div class="nw-modal-preview-balance-label">Outstanding Balance</div>':'';
const utilBarEl=isCreditCard&&creditLimit>0?`<div class="nw-card-util-wrap"><div class="nw-card-util-bar"><div class="nw-card-util-fill" style="width:${usedPct}%;background:${utilColor}"></div></div><div class="nw-card-util-labels"><span>${fmtShort(balance)} used</span><span>${fmtShort(available??0)} left of ${fmtShort(creditLimit)}</span></div></div>`:'';
const _matIcon=materialLabel==='Platinum'?'💎':materialLabel==='Gold'?'✨':'💳';
const pillLabel=`${_matIcon} ${isCreditCard?'Credit Card':'Asset Account'}`;
const fmtBal=fmt(balance);const balHtml=fmtBal.startsWith('₱')?`<span class="nw-card-currency">₱</span>${fmtBal.slice(1)}`:fmtBal;
const dotsEl='<div class="nw-card-dots">•••• •••• ••••</div>';
const contactlessEl=isCreditCard?`<div class="nw-card-contactless"><svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="17" r="1.8" fill="rgba(255,255,255,.9)"/><path d="M7.5 13A6.5 6.5 0 0 1 12 11a6.5 6.5 0 0 1 4.5 2" stroke="rgba(255,255,255,.75)" stroke-width="1.8" stroke-linecap="round" fill="none"/><path d="M4.5 9.5A10.5 10.5 0 0 1 12 7a10.5 10.5 0 0 1 7.5 2.5" stroke="rgba(255,255,255,.45)" stroke-width="1.8" stroke-linecap="round" fill="none"/></svg></div>`:'';
const topRight=networkBadge?`<div class="nw-modal-preview-top-right">${networkBadge}<span class="nw-modal-preview-pill">${pillLabel}</span></div>`:`<span class="nw-modal-preview-pill">${pillLabel}</span>`;
mount.innerHTML=`<div class="nw-modal-preview-shell"><div class="nw-modal-preview-label">Preview</div><div class="nw-modal-preview-card ${materialClass}" style="--preview-bg:${brand.bg}"><div class="nw-card-deco nw-card-deco-1"></div><div class="nw-card-deco nw-card-deco-2"></div>${contactlessEl}<div class="nw-modal-preview-top"><div class="nw-modal-preview-logo">${nwBrandLogoHtml(brand,icon)}</div>${topRight}</div>${chipEl}${balanceLabelEl}<div class="nw-modal-preview-balance">${balHtml}</div><div class="nw-modal-preview-name">${esc(rawName||'New account')}</div>${dotsEl}${metaLine?`<div class="nw-modal-preview-meta-line">${esc(metaLine)}</div>`:''}${changeHtml}${utilBarEl}${chips?`<div class="nw-modal-preview-meta">${chips}</div>`:''}</div></div>`;}
function getSpendValidationState(account,amount,refundedAmount=0){const normalizedAmount=Math.max(roundMoney(amount),0);const acc=nwAccounts.find(a=>a.key===account);if(acc?.accountType==='credit_card'){const current=roundMoney(nwBalances[account]||0);const creditLimit=roundMoney(acc.creditLimit||0);const available=creditLimit>0?roundMoney(Math.max(creditLimit-current,0)):Infinity;const hasEnough=creditLimit<=0||normalizedAmount<=available;return{current,available,after:roundMoney(current+normalizedAmount),amount:normalizedAmount,hasEnough,isCreditCard:true,creditLimit};}const current=roundMoney(nwBalances[account]||0);const available=roundMoney(current+Math.max(Number(refundedAmount||0),0));const after=roundMoney(available-normalizedAmount);return{current,available,after,amount:normalizedAmount,hasEnough:Math.round(normalizedAmount*100)<=Math.round(available*100)}}
...
function getDebtPaymentDraft(debt){
  const rawAmount=roundMoney(parseFloat(document.getElementById('dp-amount')?.value)||0);
  const remaining=Math.max(Number(debt?.total||0),0);
  const amount=roundMoney(Math.min(rawAmount,remaining));
  const fee=Math.max(roundMoney(parseFloat(document.getElementById('dp-fee')?.value)||0),0);
  return{rawAmount,amount,fee,totalDeduction:amount>0?roundMoney(amount+fee):0,remaining,after:roundMoney(Math.max(remaining-amount,0))};
}
function setDebtPaymentAmount(kind){
  const debt=debts.find(d=>d.id===activeDebtPaymentDebtId);if(!debt)return;
  const remaining=Math.max(Number(debt.total||0),0);
  let amount=remaining;
  if(kind==='min')amount=Number(debt.minDue||debt.payment||remaining);
  else if(kind==='planned')amount=Number(debt.payment||debt.minDue||remaining);
  amount=roundMoney(Math.min(Math.max(amount,0),remaining));
  document.getElementById('dp-amount').value=amount||'';
  updateDebtPaymentPreview();
}
function renderDebtPaymentQuickAmounts(debt){
  const wrap=document.getElementById('dp-quick-amounts');if(!wrap||!debt)return;
  const remaining=Math.max(Number(debt.total||0),0);
  const chips=[
    {label:'Minimum',value:Math.min(Number(debt.minDue||debt.payment||0),remaining),kind:'min'},
    {label:'Planned',value:Math.min(Number(debt.payment||debt.minDue||0),remaining),kind:'planned'},
    {label:'Clear',value:remaining,kind:'clear'}
  ].filter(c=>c.value>0);
  wrap.innerHTML=chips.map(c=>`<button class="quick-chip" type="button" onclick="setDebtPaymentAmount('${c.kind}')">${c.label} ${fmtShort(c.value)}</button>`).join('');
}
function renderDebtPaymentImpact(debt){
  const wrap=document.getElementById('dp-impact');if(!wrap||!debt)return;
  const draft=getDebtPaymentDraft(debt);
  const summary=getDebtPaymentSummary(debt.id);
  const totalPaidBefore=Number(summary.total||0);
  const original=Math.max(draft.remaining+totalPaidBefore,1);
  const beforePct=Math.min(Math.max(totalPaidBefore/original*100,0),100);
  const afterPct=Math.min(Math.max((totalPaidBefore+draft.amount)/original*100,0),100);
  const monthlyMarked=document.getElementById('dp-mark-paid')?.value!=='no';
  const projected= draft.after>0 ? getDebtPayoffProjection(draft.after,Number(debt.payment||draft.amount||0),Number(debt.interest||0)) : null;
  const payoffText=draft.after<=0?'Cleared after this payment':projected?.isViable&&projected.months?`About ${projected.months} month${projected.months===1?'':'s'} left at current plan`:'Set a monthly plan to estimate payoff';
  wrap.innerHTML=`<div class="debt-payment-impact">
    <div class="debt-payment-impact-head">
      <div><span>PAYMENT IMPACT</span><strong>${fmt(draft.amount)}</strong></div>
      <div><span>REMAINING AFTER</span><strong class="${draft.after<=0?'good':''}">${fmt(draft.after)}</strong></div>
    </div>
    <div class="debt-payment-progress"><div style="width:${Math.max(afterPct, beforePct>0?beforePct:0)}%"></div></div>
    <div class="debt-payment-impact-meta"><span>${Math.round(afterPct)}% cleared after payment</span><span>${payoffText}</span></div>
    <div class="debt-payment-impact-tags"><span>${monthlyMarked?'Counts for this month':'Not marked monthly'}</span>${draft.fee>0?`<span>Fee ${fmt(draft.fee)}</span>`:''}${draft.rawAmount>draft.amount&&draft.amount>0?'<span>Trimmed to remaining balance</span>':''}</div>
  </div>`;
}
function getGoalContributionDraft(goal){
  const rawAmount=roundMoney(parseFloat(document.getElementById('gc-amount')?.value)||0);
  const target=Number(goal?.target||0);
  const current=Number(goal?.current||0);
  const remaining=target>0?roundMoney(Math.max(target-current,0)):Infinity;
  const amount=roundMoney(Math.min(rawAmount,remaining));
  const after=roundMoney(current+Math.max(amount,0));
  const afterRemaining=target>0?roundMoney(Math.max(target-after,0)):0;
  const beforePct=target>0?Math.min(Math.round(current/target*100),100):0;
  const afterPct=target>0?Math.min(Math.round(after/target*100),100):0;
  return{rawAmount,amount,target,current,remaining,after,afterRemaining,beforePct,afterPct};
}
function setGoalContributionAmount(kind){
  const goal=goals.find(g=>g.id===activeGoalContributionGoalId);if(!goal)return;
  const target=Number(goal.target||0);
  const current=Number(goal.current||0);
  const remaining=target>0?roundMoney(Math.max(target-current,0)):0;
  let amount=remaining;
  if(kind==='planned')amount=Number(goal.monthly||remaining);
  else if(kind==='quarter')amount=Math.ceil((remaining*.25)/100)*100;
  amount=roundMoney(Math.min(Math.max(amount,0),remaining||amount));
  document.getElementById('gc-amount').value=amount||'';
  updateGoalContributionPreview();
}
function renderGoalContributionQuickAmounts(goal){
  const wrap=document.getElementById('gc-quick-amounts');if(!wrap||!goal)return;
  const target=Number(goal.target||0);
  const current=Number(goal.current||0);
  const remaining=target>0?roundMoney(Math.max(target-current,0)):0;
  const values=[
    {label:'Planned',value:Number(goal.monthly||0),kind:'planned'},
    {label:'25% left',value:remaining>0?Math.ceil((remaining*.25)/100)*100:0,kind:'quarter'},
    {label:'Complete',value:remaining,kind:'complete'}
  ].filter(chip=>chip.value>0);
  const seen=new Set();
  wrap.innerHTML=values.filter(chip=>{const key=roundMoney(Math.min(chip.value,remaining||chip.value));if(seen.has(key))return false;seen.add(key);chip.value=key;return true;}).map(chip=>`<button class="quick-chip" type="button" onclick="setGoalContributionAmount('${chip.kind}')">${chip.label} ${fmtShort(chip.value)}</button>`).join('');
}
function renderGoalContributionImpact(goal){
  const wrap=document.getElementById('gc-impact');if(!wrap||!goal)return;
  const draft=getGoalContributionDraft(goal);
  const etaText=draft.afterRemaining<=0?'Goal completed after this contribution':goal.monthly>0?`About ${Math.ceil(draft.afterRemaining/Number(goal.monthly||1))} month${Math.ceil(draft.afterRemaining/Number(goal.monthly||1))===1?'':'s'} left at planned pace`:'Set monthly target to estimate finish';
  wrap.innerHTML=`<div class="goal-contribution-impact">
    <div class="goal-contribution-impact-head">
      <div><span>CONTRIBUTION</span><strong>${fmt(draft.amount)}</strong></div>
      <div><span>SAVED AFTER</span><strong class="${draft.afterRemaining<=0?'good':''}">${fmt(draft.after)}</strong></div>
    </div>
    <div class="goal-contribution-progress"><div style="width:${Math.max(draft.afterPct,draft.beforePct)}%"></div></div>
    <div class="goal-contribution-impact-meta"><span>${draft.afterPct}% funded after saving</span><span>${etaText}</span></div>
    <div class="goal-contribution-impact-tags"><span>${fmt(draft.afterRemaining)} remaining</span>${draft.rawAmount>draft.amount&&draft.amount>0?'<span>Trimmed to remaining target</span>':''}</div>
  </div>`;
}
function isExternalTransfer(t){return t&&t.mode==='external'}
function getTransferDestinationName(t){return isExternalTransfer(t)?(t.externalTo||'Other bank account'):getAccountInfo(t.to).name}
function updateTransferModeUI(){const mode=document.getElementById('t-mode')?.value||'internal';const toWrap=document.getElementById('t-to-wrap');const externalWrap=document.getElementById('t-external-wrap');if(toWrap)toWrap.style.display=mode==='external'?'none':'';if(externalWrap)externalWrap.style.display=mode==='external'?'':'none'}
function updateTransferSummary(){const mode=document.getElementById('t-mode')?.value||'internal';const from=document.getElementById('t-from')?.value||'';const to=document.getElementById('t-to')?.value||'';const externalTo=(document.getElementById('t-external-to')?.value||'').trim();const amount=roundMoney(parseFloat(document.getElementById('t-amount')?.value)||0);const fee=roundMoney(parseFloat(document.getElementById('t-fee')?.value)||0);const summary=document.getElementById('transfer-summary');if(!summary)return;const fromName=from?getAccountInfo(from).name:'Source account';const toName=mode==='external'?(externalTo||'Other bank account'):(to?getAccountInfo(to).name:'Destination account');const total=roundMoney(Math.max(0,amount)+Math.max(0,fee));const fromAfter=from?roundMoney(Number(nwBalances[from]||0)-total):0;const toAfter=mode==='external'?null:(to?roundMoney(Number(nwBalances[to]||0)+Math.max(0,amount)):0);summary.innerHTML=`<div style="font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--text3);margin-bottom:6px">Transfer summary</div><div><strong>Transfer amount:</strong> ${fmt(amount)}</div><div><strong>Fee:</strong> ${fmt(fee)}</div><div><strong>Total deducted from ${esc(fromName)}:</strong> ${fmt(total)}</div><div><strong>Sent to ${esc(toName)}:</strong> ${fmt(amount)}</div><div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border)"><strong>${esc(fromName)} after transfer:</strong> <span style="color:${fromAfter<0?'var(--red)':'inherit'};font-weight:${fromAfter<0?800:400}">${fmt(fromAfter)}${fromAfter<0?' ⚠️':''}</span>${mode==='external'?'<br><span>This will not increase any tracked asset account.</span>':`<br><strong>${esc(toName)} after transfer:</strong> ${fmt(toAfter)}`}</div>${fee>0?'<div style="margin-top:6px;color:var(--text3)">The fee will be logged as a separate Transfer Fees expense.</div>':''}`;}
function openTransferModal(prefillFrom=''){if(!nwAccounts||!nwAccounts.length){showAlert('Add at least one account to transfer money.');return;}const fromEl=document.getElementById('t-from');const toEl=document.getElementById('t-to');const opts=nwAccounts.map(a=>`<option value="${a.key}">${a.icon} ${esc(a.name)}</option>`).join('');fromEl.innerHTML=opts;toEl.innerHTML=opts;const first=prefillFrom&&nwAccounts.find(a=>a.key===prefillFrom)?prefillFrom:nwAccounts[0].key;let second=(nwAccounts.find(a=>a.key!==first)||nwAccounts[0]).key;if(second===first&&nwAccounts[1])second=nwAccounts[1].key;fromEl.value=first;toEl.value=second;document.getElementById('t-mode').value=nwAccounts.length<2?'external':'internal';document.getElementById('t-date').value=todayStr;document.getElementById('t-amount').value='';document.getElementById('t-fee').value='0';document.getElementById('t-note').value='';document.getElementById('t-external-to').value='';updateTransferModeUI();updateTransferSummary();openModal('modal-transfer')}
...
function addTransfer(skipFutureDateCheck=false){const mode=document.getElementById('t-mode')?.value||'internal';const from=document.getElementById('t-from').value;const to=document.getElementById('t-to').value;const externalTo=(document.getElementById('t-external-to')?.value||'').trim();const _rawTAmt=(document.getElementById('t-amount').value||'');if(/[eE]/.test(_rawTAmt)){showAlert('Please enter a plain number — scientific notation (e.g. 1e5) is not accepted.');return;}const amount=roundMoney(parseFloat(_rawTAmt)||0);const fee=roundMoney(parseFloat(document.getElementById('t-fee').value)||0);const date=document.getElementById('t-date').value;const note=document.getElementById('t-note').value;if(!from){showAlert('Choose a source account.');return;}if(mode==='internal'&&(!to||from===to)){showAlert('Choose two different accounts.');return;}if(mode==='external'&&!externalTo){showAlert('Enter the recipient bank or account.');return;}if(!date||!amount||amount<=0){showAlert('Enter date and amount.');return;}if(date>todayStr&&!skipFutureDateCheck){showConfirm('Date is in the future ('+date+'). Add transfer anyway?',()=>addTransfer(true));return;}if(fee<0){showAlert('Fee cannot be negative.');return;}if(fee>amount){showAlert('Transfer fee ('+fmt(fee)+') cannot exceed the transfer amount ('+fmt(amount)+').');return;}const totalDeduction=roundMoney(amount+fee);if((Number(nwBalances[from]||0))<totalDeduction){showAlert('Not enough balance in the selected source account.');return;}adjustAccountBalance(from,-totalDeduction);if(mode==='internal')adjustAccountBalance(to,amount);const transferRecord=stampRecord({id:nextTransferId++,mode,from,to:mode==='internal'?to:'',externalTo:mode==='external'?externalTo:'',amount,fee,date,note});transfers.unshift(transferRecord);let feeEntryId=null,outflowEntryId=null;if(mode==='external'){outflowEntryId=nextId;entries.unshift(stampRecord({id:nextId++,date,category:'External Transfer',amount,account:from,note:`Sent to ${externalTo}${note?` - ${note}`:''}`,isExternalTransfer:true,transferId:transferRecord.id}));transferRecord.outflowEntryId=outflowEntryId}if(fee>0){feeEntryId=nextId;entries.unshift(stampRecord({id:nextId++,date,category:'Transfer Fees',amount:fee,account:from,note:`Transfer fee: ${getAccountInfo(from).name} -> ${getTransferDestinationName(transferRecord)}${note?` - ${note}`:''}`}));transferRecord.feeEntryId=feeEntryId}const hadPrevUndo=!!lastTransferUndo;lastTransferUndo={transfer:transferRecord,totalDeduction,feeEntryId,outflowEntryId};document.getElementById('t-amount').value='';document.getElementById('t-fee').value='0';document.getElementById('t-note').value='';document.getElementById('t-external-to').value='';closeModal('modal-transfer');saveData();render();showActionToast(`${fmt(amount)} transferred`,hadPrevUndo?`${getAccountInfo(from).name} → ${getTransferDestinationName(transferRecord)} · Previous undo replaced`:`${getAccountInfo(from).name} -> ${getTransferDestinationName(transferRecord)}${fee>0?' - Fee '+fmt(fee):''}`,'Transfer',{showUndo:true,duration:5200})}
function deleteTransferWithConfirm(id){const t=transfers.find(x=>x.id===id);if(!t)return;showConfirm(`Delete transfer: ${getAccountInfo(t.from).name} -> ${getTransferDestinationName(t)} - ${fmt(t.amount)}?\n\nThis will restore the affected account balances.`,()=>{deleteTransfer(id);showActionToast('Transfer deleted',`${getAccountInfo(t.from).name||'?'} -> ${getTransferDestinationName(t)}`,'Deleted');},'Delete',true);}
function renderTransferHistory(){const wrap=document.getElementById('transfer-history');if(!wrap)return;if(!transfers.length){wrap.innerHTML='<div class="empty"><div class="empty-text">No transfers yet.</div></div>';return;}wrap.innerHTML=`<div class="tx-list">${transfers.slice(0,8).map(t=>`<div class="tx-item"><div class="tx-icon" style="width:32px;height:32px;border-radius:9px;background:var(--surface2);font-size:14px;flex-shrink:0;display:flex;align-items:center;justify-content:center">T</div><div class="tx-info"><div class="tx-name" style="font-size:12px;font-weight:600;color:var(--text1)">${esc(getAccountInfo(t.from).name)} -> ${esc(getTransferDestinationName(t))}</div><div class="tx-meta" style="font-size:10px">${esc(t.date||'')} - ${isExternalTransfer(t)?'Other bank':'My account'} - ${t.fee&&Number(t.fee)>0?`Fee ${fmt(t.fee)}`:'No fee'}${t.note?` - ${esc(t.note)}`:''}</div></div><div class="tx-amount" style="font-size:13px;font-weight:600;color:var(--text2)">${fmt(t.amount)}</div><button class="btn-icon tx-delete" onclick="deleteTransferWithConfirm(${t.id})" style="border:none;background:none;color:var(--red);font-size:12px;cursor:pointer;padding:4px 6px;margin-left:2px" title="Delete">x</button></div>`).join('')}</div>`;}
...
function findTransferOutflowEntryId(transfer){if(transfer.outflowEntryId&&entries.some(e=>e.id===transfer.outflowEntryId))return transfer.outflowEntryId;const match=entries.find(e=>e.isExternalTransfer&&e.transferId===transfer.id);return match?match.id:null}
function deleteTransfer(id){const t=transfers.find(x=>x.id===id);if(!t)return;const feeEntryId=findTransferFeeEntryId(t);const outflowEntryId=findTransferOutflowEntryId(t);adjustAccountBalance(t.from,Number(t.amount||0)+Number(t.fee||0));if(!isExternalTransfer(t))adjustAccountBalance(t.to,-Number(t.amount||0));if(feeEntryId)entries=entries.filter(e=>e.id!==feeEntryId);if(outflowEntryId)entries=entries.filter(e=>e.id!==outflowEntryId);transfers=transfers.filter(x=>x.id!==id);if(lastTransferUndo?.transfer?.id===id)lastTransferUndo=null;saveData();render()}
function migrateExternalTransferHistoryEntries(){let changed=false;if(budgets['External Transfer']===undefined){budgets['External Transfer']=0;changed=true}transfers.filter(isExternalTransfer).forEach(t=>{if(findTransferOutflowEntryId(t))return;const outflowEntryId=nextId++;entries.unshift(stampRecord({id:outflowEntryId,date:t.date,category:'External Transfer',amount:Number(t.amount||0),account:t.from,note:`Sent to ${getTransferDestinationName(t)}${t.note?` - ${t.note}`:''}`,isExternalTransfer:true,transferId:t.id,time:t.time,createdAt:t.createdAt}));t.outflowEntryId=outflowEntryId;changed=true});if(changed)saveData()}
...
function skipOnboarding(){localStorage.setItem('ft_onboarded','1');localStorage.setItem('ft_show_gs','1');document.getElementById('onboard-overlay').classList.remove('show');render();}
function dismissGettingStartedCard(){localStorage.removeItem('ft_show_gs');const el=document.getElementById('gs-card');if(el)el.innerHTML='';}
function finishOnboarding(){
  ...
  nwBalances={};
  nwAccounts.forEach(a=>{const found=onboardAccounts.find(x=>x.key===a.key);nwBalances[a.key]=found?roundMoney(found.balance||0):0});
...
function openEntryEdit(id){const e=entries.find(x=>x.id===id);if(!e)return;if(e.isExternalTransfer&&e.transferId){showAlert('Edit this from Transfer History instead.');return;}if((e.isDebtPayment||e.isDebtPaymentFee)&&e.debtPaymentId){showAlert('Edit the linked debt payment instead.');return;}if(e.isGoalContribution&&e.goalContributionId){showAlert('Edit the linked goal contribution instead.');return;}editingEntryId=id;buildCatSelect('me-cat');document.getElementById('me-date').value=e.date;document.getElementById('me-amount').value=e.amount;document.getElementById('me-note').value=e.note||'';document.getElementById('me-cat').value=e.category;buildAccountSelect('me-account',true,true);document.getElementById('me-account').value=e.account||'';toggleEditCustom();updateEntryEditPreview();openModal('modal-edit-entry')}
function saveEntryEdit(){const e=entries.find(x=>x.id===editingEntryId);if(!e)return;let cat=document.getElementById('me-cat').value;const date=document.getElementById('me-date').value;const _rawMEAmt=(document.getElementById('me-amount').value||'');if(/[eE]/.test(_rawMEAmt)){showAlert('Please enter a plain number — scientific notation (e.g. 1e5) is not accepted.');return;}const amount=roundMoney(parseFloat(_rawMEAmt)||0);const note=document.getElementById('me-note').value;const account=document.getElementById('me-account').value;if(!date||!amount||amount<=0){showAlert('Enter date and amount.');return;}if(!account){showAlert('Choose an account.');return;}if(cat==='__other__'){const cn=document.getElementById('me-custom-cat').value.trim();if(!cn){showAlert('Enter category name.');return;}if(!allCats().find(c=>c.name===cn)){customCats.push({name:cn,budget:0,type:'other',group:'wants',icon:'🏷️',colorClass:'cat-default'});budgets[cn]=0}cat=cn;document.getElementById('me-custom-cat').value=''}const refundedAmount=account===e.account?Number(e.amount||0):0;const balanceState=getSpendValidationState(account,amount,refundedAmount);if(!balanceState.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(balanceState.available)}`);return;}if(isCCAccount(e.account)){adjustAccountBalance(e.account,-e.amount);const _ccA0=nwAccounts.find(a=>a.key===e.account);if(_ccA0?.linkedDebtId){const _ld0=debts.find(d=>String(d.id)===String(_ccA0.linkedDebtId));if(_ld0)_ld0.total=roundMoney(Math.max(0,Number(_ld0.total||0)-e.amount));}}else{adjustAccountBalance(e.account,e.amount);}e.date=date;e.amount=amount;e.note=note;e.category=cat;e.account=account;if(isCCAccount(account)){adjustAccountBalance(account,amount);const _ccA1=nwAccounts.find(a=>a.key===account);if(_ccA1?.linkedDebtId){const _ld1=debts.find(d=>String(d.id)===String(_ccA1.linkedDebtId));if(_ld1)_ld1.total=roundMoney(Number(_ld1.total||0)+amount);}}else{adjustAccountBalance(account,-amount);}closeModal('modal-edit-entry');saveData();render()}
function deleteEditingEntry(){if(editingEntryId===null)return;if(deleteEntry(editingEntryId))closeModal('modal-edit-entry')}
function duplicateEditingEntry(){const e=entries.find(x=>x.id===editingEntryId);if(!e)return;const account=e.account||'';const amount=roundMoney(e.amount||0);const balanceState=getSpendValidationState(account,amount);if(account&&!balanceState.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(balanceState.available)}`);return;}const duplicate=stampRecord({...e,id:nextId++,date:todayStr,note:e.note?e.note+' (copy)':'Copy'});entries.unshift(duplicate);if(isCCAccount(account)){adjustAccountBalance(account,amount);const _ccAd=nwAccounts.find(a=>a.key===account);if(_ccAd?.linkedDebtId){const _ldd=debts.find(d=>String(d.id)===String(_ccAd.linkedDebtId));if(_ldd)_ldd.total=roundMoney(Number(_ldd.total||0)+amount);}}else{adjustAccountBalance(account,-amount);}rememberExpensePattern(duplicate);closeModal('modal-edit-entry');saveData();render();showActionToast(`${fmt(amount)} duplicated`,duplicate.category,'🧾')}
...
function saveIncomeEdit(){const i=incomes.find(x=>x.id===editingIncomeId);if(!i)return;if(i.isSalaryDeposit){showAlert('Scheduled salary deposits cannot be edited here. Delete the deposit first if you need to log it again correctly.');return;}const date=document.getElementById('mi-date').value;const _rawMIAmt=(document.getElementById('mi-amount').value||'');if(/[eE]/.test(_rawMIAmt)){showAlert('Please enter a plain number — scientific notation (e.g. 1e5) is not accepted.');return;}const amount=roundMoney(parseFloat(_rawMIAmt)||0);const source=document.getElementById('mi-source').value;const account=document.getElementById('mi-account').value;const note=document.getElementById('mi-note').value;if(!date||!amount||amount<=0){showAlert('Enter date and amount.');return;}adjustAccountBalance(i.account,-i.amount);i.date=date;i.amount=amount;i.source=source;i.account=account;i.note=note;adjustAccountBalance(account,amount);closeModal('modal-edit-income');saveData();render()}
...
function toggleRecurringType(){
  const type=document.getElementById('r-type')?.value||'bill';
  const catWrap=document.getElementById('r-cat-wrap');
  const xferFields=document.getElementById('r-transfer-fields');
  if(catWrap)catWrap.style.display=type==='bill'?'block':'none';
  if(xferFields)xferFields.style.display=type==='transfer'?'block':'none';
  if(type==='transfer'){
    buildAccountSelect('r-from',true);buildAccountSelect('r-to',true);
  }
}
function addRecurring(){const type=document.getElementById('r-type').value;const name=document.getElementById('r-name').value.trim();const amount=parseFloat(document.getElementById('r-amount').value)||0;const day=parseInt(document.getElementById('r-day').value)||0;const category=document.getElementById('r-cat').value;const from=document.getElementById('r-from')?.value||'';const to=document.getElementById('r-to')?.value||'';if(!name||amount<=0||day<1||day>31){showAlert('Enter a valid name, amount, and day.');return;}if(type==='bill'&&!category){showAlert('Choose a category.');return;}if(type==='transfer'&&(!from||!to||from===to)){showAlert('Choose two different accounts for the transfer.');return;}recurring.unshift({id:nextRecurringId++,type,name,amount,day,category:type==='bill'?category:'',from:type==='transfer'?from:'',to:type==='transfer'?to:'',lastPaid:''});['r-name','r-amount','r-day'].forEach(id=>document.getElementById(id).value='');document.getElementById('r-type').value='bill';toggleRecurringType();closeModal('modal-add-recurring');saveData();render()}
function postRecurringItem(item,{skipSave=false}={}){
  const monthKey=currentMonthKey();const account=getDefaultAccountKey();
  if(!item)return{ok:false,message:'Recurring item not found.'};
  if(item.lastPaid===monthKey)return{ok:false,message:`${item.name} is already marked this month.`};
  if(item.type==='transfer'){
    const from=item.from||getDefaultAccountKey();const to=item.to||'';if(!from||!to)return{ok:false,message:`${item.name} is missing transfer accounts.`};
    const needed=Number(item.amount||0);if((Number(nwBalances[from]||0))<needed)return{ok:false,message:`Not enough balance in ${getAccountInfo(from).name} for ${item.name}.`};
    adjustAccountBalance(from,-needed);adjustAccountBalance(to,needed);const rec=stampRecord({id:nextTransferId++,mode:'internal',from,to,amount:needed,fee:0,date:todayStr,note:'Recurring: '+item.name});transfers.unshift(rec);lastTransferUndo={transfer:rec,totalDeduction:needed,feeEntryId:null,outflowEntryId:null};item.lastPaid=monthKey;if(!skipSave){saveData();render();showActionToast('Recurring transfer done',`${getAccountInfo(from).name} → ${getAccountInfo(to).name}`,'↔',{showUndo:true,duration:4000});}return{ok:true,type:'transfer'};
  }
  if(!account)return{ok:false,message:item.type==='bill'?'Add an account first to pay recurring bills.':'Add an account first to receive recurring income.'};
  if(item.type==='bill'){
    const balanceState=getSpendValidationState(account,Number(item.amount||0));if(!balanceState.hasEnough)return{ok:false,message:`Not enough balance in ${getAccountInfo(account).name} for ${item.name}.`};
    entries.unshift(stampRecord({id:nextId++,date:todayStr,category:item.category||'Miscellaneous / Buffer',amount:item.amount,note:'Recurring: '+item.name,account}));adjustAccountBalance(account,-item.amount);
  }else{
    incomes.unshift(stampRecord({id:nextIncId++,date:todayStr,source:item.name,amount:item.amount,note:'Recurring income',account}));adjustAccountBalance(account,item.amount);
  }
  item.lastPaid=monthKey;if(!skipSave){saveData();render();}return{ok:true,type:item.type};
}
function markRecurringPaid(id){const item=recurring.find(x=>x.id===id);const result=postRecurringItem(item);if(!result.ok)showAlert(result.message);else if(result.type!=='transfer')showActionToast('Recurring item posted',item.name,item.type==='bill'?'🧾':'💵')}
let recurringBatchSelected=new Set();
function getRecurringBatchItems(){
  const monthKey=currentMonthKey();
  return recurring.map(item=>({item,status:recurringStatus(item),due:recurringDueDate(item,monthKey)})).filter(x=>x.status.state!=='paid').sort((a,b)=>a.due-b.due);
}
function openRecurringBatchAssistant(){
  const items=getRecurringBatchItems();
  recurringBatchSelected=new Set(items.filter(({status})=>status.days<=7).map(({item})=>item.id));
  renderRecurringBatchAssistant();
  openModal('modal-recurring-batch');
}
function toggleRecurringBatchSelection(id,checked){if(checked)recurringBatchSelected.add(id);else recurringBatchSelected.delete(id);renderRecurringBatchAssistant()}
function renderRecurringBatchAssistant(){
  const summary=document.getElementById('recurring-batch-summary');const list=document.getElementById('recurring-batch-list');const confirm=document.getElementById('recurring-batch-confirm');if(!summary||!list||!confirm)return;
  const items=getRecurringBatchItems();const selected=items.filter(({item})=>recurringBatchSelected.has(item.id));
  if(!items.length){summary.innerHTML='';list.innerHTML='<div class="empty"><div class="empty-icon">🔁</div><div class="empty-text">No unpaid recurring items this month</div></div>';confirm.disabled=true;confirm.textContent='Confirm Selected';return;}
  const billTotal=selected.filter(x=>x.item.type==='bill').reduce((s,x)=>s+Number(x.item.amount||0),0);
  const incomeTotal=selected.filter(x=>x.item.type==='income').reduce((s,x)=>s+Number(x.item.amount||0),0);
  const transferTotal=selected.filter(x=>x.item.type==='transfer').reduce((s,x)=>s+Number(x.item.amount||0),0);
  const defaultAccount=getDefaultAccountKey();
  const afterDefault=roundMoney(Number(nwBalances[defaultAccount]||0)-billTotal+incomeTotal);
  const impactHtml=defaultAccount?`<div class="recurring-batch-impact">${esc(getAccountInfo(defaultAccount).name)} after bills/income: <strong style="color:${afterDefault<0?'var(--red)':'inherit'}">${fmt(afterDefault)}</strong></div>`:'';
  const rowsHtml=items.map(({item,status,due})=>{
    const checked=recurringBatchSelected.has(item.id);
    const icon=item.type==='transfer'?'↔':item.type==='bill'?'🧾':'💵';
    const dueLabel=due.toLocaleDateString('en-PH',{month:'short',day:'numeric'});
    let meta='Income';
    if(item.type==='transfer')meta=`${esc(getAccountInfo(item.from).name)} → ${esc(getAccountInfo(item.to).name)}`;
    else if(item.type==='bill')meta=esc(item.category||'Bill');
    return `<label class="recurring-batch-row ${checked?'selected':''}"><input type="checkbox" ${checked?'checked':''} onchange="toggleRecurringBatchSelection(${item.id},this.checked)"><span class="recurring-batch-icon">${icon}</span><span class="recurring-batch-main"><strong>${esc(item.name)}</strong><span>${meta} · Due ${dueLabel} · ${esc(status.label)}</span></span><span class="recurring-batch-amount">${fmt(item.amount)}</span></label>`;
  }).join('');
  summary.innerHTML=`<div class="recurring-batch-summary"><div><strong>${selected.length}</strong><span>selected</span></div><div><strong>${fmt(billTotal)}</strong><span>bills</span></div><div><strong>${fmt(incomeTotal)}</strong><span>income</span></div><div><strong>${fmt(transferTotal)}</strong><span>transfers</span></div></div>${impactHtml}`;
  list.innerHTML=`<div class="recurring-batch-list">${rowsHtml}</div>`;
  confirm.disabled=!selected.length;confirm.textContent=selected.length?`Confirm ${selected.length}`:'Confirm Selected';
}
function confirmRecurringBatch(){
  const selectedIds=[...recurringBatchSelected];if(!selectedIds.length){showAlert('Select at least one recurring item.');return;}
  showConfirm(`Post ${selectedIds.length} recurring item${selectedIds.length===1?'':'s'} for this month?`,()=>{
    const errors=[];let posted=0;
    selectedIds.forEach(id=>{const item=recurring.find(x=>x.id===id);const result=postRecurringItem(item,{skipSave:true});if(result.ok)posted++;else errors.push(result.message);});
    saveData();closeModal('modal-recurring-batch');render();if(posted)showActionToast('Recurring batch posted',`${posted} item${posted===1?'':'s'} recorded.`,'🔁');if(errors.length)showAlert(errors.slice(0,4).join('\\n'),`${errors.length} item${errors.length===1?'':'s'} skipped`);
  },'Confirm Batch');
}
...
const _NW_ICON_GRID=`<svg width="13" height="13" viewBox="0 0 13 13" fill="currentColor"><rect x="0" y="0" width="5.5" height="5.5" rx="1.5"/><rect x="7.5" y="0" width="5.5" height="5.5" rx="1.5"/><rect x="0" y="7.5" width="5.5" height="5.5" rx="1.5"/><rect x="7.5" y="7.5" width="5.5" height="5.5" rx="1.5"/></svg>`;
function applyNwViewMode(){
  const isGrid=nwViewMode==='grid';
  document.querySelectorAll('.nw-bank-grid').forEach(el=>el.classList.toggle('nw-grid-view',isGrid));
  const btn=document.getElementById('nw-view-toggle-btn');
  if(btn)btn.innerHTML=isGrid?_NW_ICON_CAROUSEL:_NW_ICON_GRID;
}
function toggleNwView(){nwViewMode=nwViewMode==='carousel'?'grid':'carousel';saveData();applyNwViewMode();}

function toggleNotifications(){
  const panel=document.getElementById('notif-panel');
  if(!panel)return;
  renderCashflowNotification();
  renderDueSoonNotification();
  panel.classList.toggle('show');
  const open=panel.classList.contains('show');
  const bell=document.getElementById('tour-target-bell');
...
function updateGoalContributionPreview(){const wrap=document.getElementById('gc-balance-preview');const goal=goals.find(g=>g.id===activeGoalContributionGoalId);if(!wrap||!goal)return;const account=document.getElementById('gc-account')?.value||'';const draft=getGoalContributionDraft(goal);const extraLines=draft.rawAmount>draft.amount&&draft.amount>0?[`<div>Only <strong>${fmt(draft.amount)}</strong> is needed to complete this goal.</div>`]:[];if(draft.amount>0)extraLines.push(`<div>Applied to goal: <strong>${fmt(draft.amount)}</strong></div>`);renderGoalContributionImpact(goal);renderSpendBalancePreview({wrapId:'gc-balance-preview',buttonId:'gc-save-btn',title:'Contribution balance preview',account,amount:draft.amount,submitLabel:'Save',afterLabel:'After this contribution',extraLines,okMessage:'Enough balance for this contribution.',warningMessage:'Not enough balance in this account.'})}
function updateDebtPaymentPreview(){const wrap=document.getElementById('dp-balance-preview');const debt=debts.find(d=>d.id===activeDebtPaymentDebtId);if(!wrap||!debt)return;const account=document.getElementById('dp-account')?.value||'';const draft=getDebtPaymentDraft(debt);const extraLines=draft.rawAmount>draft.amount&&draft.amount>0?[`<div>Only <strong>${fmt(draft.amount)}</strong> will be applied because that is the remaining debt balance.</div>`]:[];if(draft.amount>0)extraLines.push(`<div>Applied to debt: <strong>${fmt(draft.amount)}</strong></div>`);if(draft.amount>0&&draft.fee>0){extraLines.push(`<div>Bank transfer fee: <strong>${fmt(draft.fee)}</strong></div>`);extraLines.push(`<div>Total deducted today: <strong>${fmt(draft.totalDeduction)}</strong></div>`)}renderDebtPaymentImpact(debt);renderSpendBalancePreview({wrapId:'dp-balance-preview',buttonId:'dp-save-btn',title:'Payment balance preview',account,amount:draft.totalDeduction,submitLabel:'Save Payment',afterLabel:draft.fee>0?'After payment and fee':'After this payment',extraLines,okMessage:draft.fee>0?'Enough balance for this payment and fee.':'Enough balance for this payment.',warningMessage:draft.fee>0?'Not enough balance in this account for the payment and fee.':'Not enough balance in this account.'})}
...
function resetExpenseForm(keepCategory=false){document.getElementById('f-amount').value='';if(!keepCategory){document.getElementById('f-cat').selectedIndex=0;toggleCustom();}document.getElementById('f-date').value=todayStr;document.getElementById('f-note').value='';const tagsEl=document.getElementById('f-tags');if(tagsEl)tagsEl.value='';const tagSugEl=document.getElementById('tag-suggestions');if(tagSugEl)tagSugEl.innerHTML='';updateAddPreviews();renderExpenseSuggestions();}
...
function addEntryCore(stayOnAdd=false,afterSave=null,{skipFutureDateCheck=false}={}){const date=document.getElementById('f-date').value||todayStr;const _rawFAmt=(document.getElementById('f-amount').value||'');if(/[eE]/.test(_rawFAmt)){showAlert('Please enter a plain number — scientific notation (e.g. 1e5) is not accepted.');return;}const amount=roundMoney(parseFloat(_rawFAmt)||0);let category=document.getElementById('f-cat').value;const account=document.getElementById('f-account').value;const note=document.getElementById('f-note').value.trim();const rawTags=(document.getElementById('f-tags')?.value||'');const tags=rawTags.split(',').map(t=>t.trim().toLowerCase()).filter(Boolean).slice(0,3);if(category==='__other__'){category=ensureCustomExpenseCategory(document.getElementById('f-custom-cat').value,_selectedCatGroup);if(!category){showAlert('Enter custom category name.');return;}document.getElementById('f-custom-cat').value='';document.getElementById('custom-cat-group-wrap').style.display='none';}if(!date){showAlert('Date is required.');return;}if(!amount||amount<=0){showAlert('Please enter an amount greater than ₱0.');return;}if(!category){showAlert('Please select a category.');return;}if(!account){showAlert('Please select an account.');return;}if(date>todayStr&&!skipFutureDateCheck){showConfirm('Date is in the future ('+date+'). Log this expense anyway?',()=>addEntryCore(stayOnAdd,afterSave,{skipFutureDateCheck:true}));return;}const balanceState=getSpendValidationState(account,amount);if(!balanceState.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(balanceState.available)}`);return;}const entry=stampRecord({id:nextId++,date,amount,category,account,note,...(tags.length?{tags}:{})});if(checkLikelyDuplicateExpense(entry)){showConfirm('This looks similar to a recent expense. Add anyway?',()=>{entries.unshift(entry);if(isCCAccount(account)){adjustAccountBalance(account,amount);const _ccA=nwAccounts.find(a=>a.key===account);if(_ccA?.linkedDebtId){const _ld=debts.find(d=>String(d.id)===String(_ccA.linkedDebtId));if(_ld)_ld.total=roundMoney(Number(_ld.total||0)+amount);}}else{adjustAccountBalance(account,-amount);}rememberExpensePattern(entry);rememberTags(tags);saveData();resetExpenseForm(true);render();showActionToast(`${fmt(amount)} expense added`,`${category} · ${getAccountInfo(account).name}`,'🧾');if(typeof afterSave==='function'){afterSave(entry);return}});return;}entries.unshift(entry);if(isCCAccount(account)){adjustAccountBalance(account,amount);const _ccA=nwAccounts.find(a=>a.key===account);if(_ccA?.linkedDebtId){const _ld=debts.find(d=>String(d.id)===String(_ccA.linkedDebtId));if(_ld)_ld.total=roundMoney(Number(_ld.total||0)+amount);}}else{adjustAccountBalance(account,-amount);}rememberExpensePattern(entry);rememberTags(tags);saveData();resetExpenseForm(true);render();showActionToast(`${fmt(amount)} expense added`,`${category} · ${getAccountInfo(account).name}`,'🧾');if(typeof afterSave==='function'){afterSave(entry);return}}
function addIncomeCore(stayOnAdd=false,afterSave=null,{skipFutureDateCheck=false}={}){const date=document.getElementById('inc-date').value||todayStr;const _rawIAmt=(document.getElementById('inc-amount').value||'');if(/[eE]/.test(_rawIAmt)){showAlert('Please enter a plain number — scientific notation (e.g. 1e5) is not accepted.');return;}const amount=roundMoney(parseFloat(_rawIAmt)||0);const source=document.getElementById('inc-source').value;const account=document.getElementById('inc-account').value;const note=document.getElementById('inc-note').value.trim();if(!date){showAlert('Date is required.');return;}if(!amount||amount<=0){showAlert('Please enter an amount greater than ₱0.');return;}if(!source){showAlert('Please enter an income source.');return;}if(!account){showAlert('Please select an account.');return;}if(date>todayStr&&!skipFutureDateCheck){showConfirm('Date is in the future ('+date+'). Log this income anyway?',()=>addIncomeCore(stayOnAdd,afterSave,{skipFutureDateCheck:true}));return;}const income=stampRecord({id:nextIncId++,date,source,amount,note,account});incomes.unshift(income);adjustAccountBalance(account,amount);rememberIncomePattern(income);saveData();resetIncomeForm(true);render();showActionToast(`${fmt(amount)} income added`,`${source} · ${getAccountInfo(account).name}`,'💰');if(typeof afterSave==='function'){afterSave(income);return}if(stayOnAdd){showTab('add');return}showTab('dashboard');}
...
function buildGoalsTimelineSVG(){
  ...
  let svg=`<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:${H}px;display:block;overflow:visible" role="img" aria-label="Goals savings timeline showing ${goals.filter(g=>g.target>0).length} active goal${goals.filter(g=>g.target>0).length===1?'':'s'}"><title>Goals savings timeline</title>`;
...
function deleteEntry(id){const e=entries.find(x=>x.id===id);if(!e)return false;if(e.isExternalTransfer&&e.transferId)return deleteTransfer(e.transferId);if((e.isDebtPayment||e.isDebtPaymentFee)&&e.debtPaymentId)return deleteDebtPayment(e.debtPaymentId);if(e.isGoalContribution&&e.goalContributionId)return deleteGoalContribution(e.goalContributionId);showConfirm(`Delete this ${e.category} expense for ${fmt(e.amount)}?`,()=>{if(isCCAccount(e.account)){adjustAccountBalance(e.account,-e.amount);const _ccA=nwAccounts.find(a=>a.key===e.account);if(_ccA?.linkedDebtId){const _ld=debts.find(d=>String(d.id)===String(_ccA.linkedDebtId));if(_ld)_ld.total=roundMoney(Math.max(0,Number(_ld.total||0)-e.amount));}}else{adjustAccountBalance(e.account,e.amount);}entries=entries.filter(entry=>entry.id!==id);saveData();render();showActionToast('Expense deleted',`${e.category} · ${fmt(e.amount)}`,'🗑️');},'Delete',true);return true}
...
function renderGoalHistory(id){
  const g=goals.find(x=>x.id===id);if(!g)return;
  const list=getGoalContributions(id);
  const wrap=document.getElementById('gh-list');if(!wrap)return;
  document.getElementById('gh-title').textContent=`${g.name} Contributions`;
  if(!list.length){wrap.innerHTML='<div class="empty"><div class="empty-icon">🎯</div><div class="empty-text">No contributions yet</div></div>';return;}
  ...
  const groups={};
  list.forEach(c=>{const key=(c.date||'Undated').slice(0,7)||'Undated';(groups[key]||(groups[key]=[])).push(c);});
  const groupHtml=Object.entries(groups).map(([key,items])=>{
    const monthLabel=key==='Undated'?'Undated':new Date(`${key}-01T00:00:00`).toLocaleDateString('en-PH',{month:'long',year:'numeric'});
    const monthTotal=items.reduce((s,c)=>s+Number(c.amount||0),0);
    return`<div class="goal-history-group"><div class="goal-history-group-head"><div><strong>${esc(monthLabel)}</strong><span>${items.length} contribution${items.length===1?'':'s'}</span></div><div>${fmt(monthTotal)}</div></div>${items.map(c=>{const account=c.account?getAccountInfo(c.account).name:'No account';return`<div class="goal-history-row"><div class="goal-history-icon">🎯</div><div class="goal-history-main"><div class="goal-history-title"><strong>${fmt(c.amount)}</strong></div><div class="goal-history-meta">${esc(formatDateTime(c))} · ${esc(account)}${c.note?` · ${esc(c.note)}`:''}</div></div><button class="btn-icon goal-history-delete" onclick="deleteGoalContribution(${c.id})" title="Delete contribution" aria-label="Delete contribution">✕</button></div>`}).join('')}</div>`;
  }).join('');
  wrap.innerHTML=`<div class="goal-history-summary"><div><strong>${fmt(total)}</strong><span>logged here</span></div><div><strong>${list.length}</strong><span>contributions</span></div><div><strong>${pct}%</strong><span>goal progress</span></div><div><strong>${target>0?fmt(remaining):'No target'}</strong><span>remaining</span></div></div><div class="goal-history-progress"><div style="width:${pct}%"></div></div>${groupHtml}`;
}
function openGoalContribution(id){const g=goals.find(x=>x.id===id);if(!g)return;activeGoalContributionGoalId=id;document.getElementById('gc-amount').value=g.monthly||'';document.getElementById('gc-date').value=todayStr;buildAccountSelect('gc-account',true);document.getElementById('gc-account').value=getDefaultAccountKey();document.getElementById('gc-note').value='';renderGoalContributionQuickAmounts(g);updateGoalContributionPreview();openModal('modal-goal-contribution')}
...
function saveGoalContribution(){const g=goals.find(x=>x.id===activeGoalContributionGoalId);if(!g)return;let amount=roundMoney(parseFloat(document.getElementById('gc-amount').value)||0);const date=document.getElementById('gc-date').value;const account=document.getElementById('gc-account').value||getDefaultAccountKey();const note=document.getElementById('gc-note').value.trim();if(amount<=0||!date){showAlert('Enter a valid amount and date.');return;}if(!account){showAlert('Choose an account.');return;}const hasTarget=Number(g.target||0)>0;const remaining=hasTarget?roundMoney(Math.max(Number(g.target||0)-Number(g.current||0),0)):Infinity;if(hasTarget&&remaining===0){showAlert(`${g.name} is already fully funded. Edit the goal to increase the target if you want to save more.`);return;}function _doSaveGoalContribution(finalAmount){finalAmount=roundMoney(finalAmount);const balanceState=getSpendValidationState(account,finalAmount);if(!balanceState.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(balanceState.available)}`);return;}g.current=roundMoney(Number(g.current||0)+finalAmount);const contributionId=nextGoalContributionId++;goalContributions.unshift(stampRecord({id:contributionId,goalId:g.id,name:g.name,amount:finalAmount,date,account,note}));entries.unshift(stampRecord({id:nextId++,date,category:'Goal Contribution',amount:finalAmount,note:`Goal Contribution: ${g.name}${note?` · ${note}`:''}`,account,isGoalContribution:true,goalId:g.id,goalContributionId:contributionId}));adjustAccountBalance(account,-finalAmount);const goalDone=Number(g.current||0)>=Number(g.target||0)&&hasTarget;closeModal('modal-goal-contribution');saveData();render();showActionToast(`${fmt(finalAmount)} added to ${g.name}`,`New saved amount: ${fmt(g.current||0)}`,'🎯');if(goalDone)showMilestoneSheet({icon:'🎯',title:'Goal completed',body:`${g.name} is now fully funded.`,statLabel:'Saved total',statValue:fmt(g.current||0)});}if(hasTarget&&amount>remaining){showConfirm(`Only ${fmt(remaining)} is needed to complete this goal.\n\nThe contribution will be capped at ${fmt(remaining)} so your account balance stays accurate. Continue?`,()=>{_doSaveGoalContribution(remaining);});return;}_doSaveGoalContribution(amount);}
function deleteGoalContribution(id){const gc=goalContributions.find(x=>x.id===id);if(!gc)return false;const g=goals.find(x=>x.id===gc.goalId);showConfirm(`Delete this ${fmt(gc.amount)} contribution${g?` for ${g.name}`:''}?\n\nThis will lower saved progress by ${fmt(gc.amount)} and restore ${fmt(gc.amount)} to ${getAccountInfo(gc.account).name}. The linked Goal Contribution transaction will also be removed.`,()=>{if(g)g.current=roundMoney(Math.max(0,Number(g.current||0)-Number(gc.amount||0)));adjustAccountBalance(gc.account,Number(gc.amount||0));goalContributions=goalContributions.filter(x=>x.id!==id);entries=entries.filter(e=>e.goalContributionId!==id);saveData();render();if(document.getElementById('modal-goal-history')?.classList.contains('show')&&g)renderGoalHistory(g.id);showActionToast('Contribution deleted',`${g?.name||'Goal'} · ${fmt(gc.amount)}`,'🗑️');},'Delete Contribution',true);return true}
function openGoalHistory(id){const g=goals.find(x=>x.id===id);if(!g)return;renderGoalHistory(id);openModal('modal-goal-history')}
...
function saveGoalEdit(){const g=goals.find(x=>x.id===editingGoalId);if(!g)return;g.name=document.getElementById('mg-name').value.trim()||g.name;g.target=roundMoney(parseFloat(document.getElementById('mg-target').value)||g.target);g.current=roundMoney(parseFloat(document.getElementById('mg-current').value)||0);g.monthly=roundMoney(parseFloat(document.getElementById('mg-monthly').value)||0);g.targetDate=document.getElementById('mg-target-date').value||'';closeModal('modal-edit-goal');saveData();render()}
...
function renderDebtHistory(id){
  const debt=debts.find(d=>d.id===id);if(!debt)return;
  const list=getDebtPaymentsForDebt(id);
  const wrap=document.getElementById('dh-list');if(!wrap)return;
  document.getElementById('dh-title').textContent=`${debt.name} Payments`;
  if(!list.length){wrap.innerHTML='<div class="empty"><div class="empty-icon">💳</div><div class="empty-text">No payments yet</div></div>';return;}
  const paidTotal=list.reduce((sum,p)=>sum+Number(p.amount||0),0);
  const feeTotal=list.reduce((sum,p)=>sum+getDebtPaymentFee(p),0);
  const monthlyCount=list.filter(p=>p.markedMonthly).length;
  const groups={};
  list.forEach(p=>{const key=(p.date||'Undated').slice(0,7)||'Undated';(groups[key]||(groups[key]=[])).push(p);});
  const groupHtml=Object.entries(groups).map(([key,items])=>{
    const monthLabel=key==='Undated'?'Undated':new Date(`${key}-01T00:00:00`).toLocaleDateString('en-PH',{month:'long',year:'numeric'});
    const monthPaid=items.reduce((s,p)=>s+Number(p.amount||0),0);
    const monthFee=items.reduce((s,p)=>s+getDebtPaymentFee(p),0);
    return`<div class="debt-history-group"><div class="debt-history-group-head"><div><strong>${esc(monthLabel)}</strong><span>${items.length} payment${items.length===1?'':'s'}</span></div><div>${fmt(monthPaid)}${monthFee>0?`<span>+ ${fmt(monthFee)} fees</span>`:''}</div></div>${items.map(p=>{const fee=getDebtPaymentFee(p);const account=p.account?getAccountInfo(p.account).name:'No account';return`<div class="debt-history-row"><div class="debt-history-icon">💳</div><div class="debt-history-main"><div class="debt-history-title"><strong>${fmt(p.amount)}</strong>${p.markedMonthly?'<span>Monthly</span>':''}</div><div class="debt-history-meta">${esc(formatDateTime(p))} · ${esc(account)}${fee>0?` · Fee ${fmt(fee)}`:''}${p.note?` · ${esc(p.note)}`:''}</div></div><button class="btn-icon debt-history-delete" onclick="deleteDebtPayment(${p.id})" title="Delete payment" aria-label="Delete payment">✕</button></div>`}).join('')}</div>`;
  }).join('');
  wrap.innerHTML=`<div class="debt-history-summary"><div><strong>${fmt(paidTotal)}</strong><span>principal paid</span></div><div><strong>${fmt(feeTotal)}</strong><span>fees</span></div><div><strong>${list.length}</strong><span>payments</span></div><div><strong>${monthlyCount}</strong><span>monthly marks</span></div></div>${groupHtml}`;
}
...
function openDebtHistory(id){const debt=debts.find(d=>d.id===id);if(!debt)return;renderDebtHistory(id);openModal('modal-debt-history')}
function openDebtPayment(id){const debt=debts.find(x=>x.id===id);if(!debt)return;activeDebtPaymentDebtId=id;document.getElementById('dp-debt-name').textContent=`Payment for ${debt.name}`;document.getElementById('dp-amount').value=debt.payment||debt.total||'';document.getElementById('dp-date').value=todayStr;document.getElementById('dp-note').value='';document.getElementById('dp-fee').value='0';document.getElementById('dp-mark-paid').value='yes';buildAccountSelect('dp-account',true);document.getElementById('dp-account').value=getDefaultAccountKey();renderDebtPaymentQuickAmounts(debt);updateDebtPaymentPreview();openModal('modal-debt-payment')}
function saveDebtPayment(){const debt=debts.find(d=>d.id===activeDebtPaymentDebtId);if(!debt)return;const rawAmount=roundMoney(parseFloat(document.getElementById('dp-amount').value)||0);const fee=Math.max(roundMoney(parseFloat(document.getElementById('dp-fee').value)||0),0);const date=document.getElementById('dp-date').value;const account=document.getElementById('dp-account').value||getDefaultAccountKey();const note=document.getElementById('dp-note').value.trim();const markPaid=document.getElementById('dp-mark-paid').value==='yes';if(rawAmount<=0||!date){showAlert('Enter a valid amount and date.');return;}if(!account){showAlert('Choose an account.');return;}const amount=roundMoney(Math.min(rawAmount,Math.max(Number(debt.total||0),0)));if(amount<=0){showAlert('This debt is already fully paid.');return;}const totalDeduction=roundMoney(amount+fee);const balanceState=getSpendValidationState(account,totalDeduction);if(!balanceState.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(balanceState.available)}`);return;}debt.total=roundMoney(Math.max(0,Number(debt.total||0)-amount));if(markPaid)debt.lastPaidMonth=(date||todayStr).slice(0,7);debt.lastPaidDate=date;debt.lastPaidAmount=amount;const paymentId=nextDebtPaymentId++;debtPayments.unshift(stampRecord({id:paymentId,debtId:debt.id,name:debt.name,amount,fee,date,account,note,markedMonthly:markPaid}));if(fee>0)entries.unshift(stampRecord({id:nextId++,date,category:'Transfer Fees',amount:fee,account,note:`Debt payment fee: ${debt.name}${note?` · ${note}`:''}`,isDebtPaymentFee:true,debtId:debt.id,debtPaymentId:paymentId}));entries.unshift(stampRecord({id:nextId++,date,category:'Debt Payment',amount,note:`Debt Payment: ${debt.name}${note?` · ${note}`:''}`,account,isDebtPayment:true,debtId:debt.id,debtPaymentId:paymentId}));adjustAccountBalance(account,-totalDeduction);
  ...
  if(linkedNwAcc)nwBalances[linkedNwAcc.key]=roundMoney(Math.max(0,Number(debt.total||0)));
...
function deleteDebtPayment(paymentId){const payment=debtPayments.find(p=>p.id===paymentId);if(!payment)return false;const totalBack=getDebtPaymentTotalDeduction(payment);showConfirm(`Delete this ${fmt(payment.amount)} payment for ${payment.name}?\n\nThis will add ${fmt(payment.amount)} back to the debt balance and restore ${fmt(totalBack)} to ${getAccountInfo(payment.account).name}. Linked Debt Payment and fee transactions will also be removed.`,()=>{const debt=debts.find(d=>d.id===payment.debtId);if(debt)debt.total=roundMoney(Number(debt.total||0)+Number(payment.amount||0));adjustAccountBalance(payment.account,totalBack);debtPayments=debtPayments.filter(p=>p.id!==paymentId);entries=entries.filter(e=>e.debtPaymentId!==paymentId);refreshDebtPaymentDerivedState(payment.debtId);saveData();render();if(document.getElementById('modal-debt-history')?.classList.contains('show'))renderDebtHistory(payment.debtId);showActionToast('Debt payment deleted',`${payment.name} · ${fmt(payment.amount)}${getDebtPaymentFee(payment)>0?` · Fee ${fmt(getDebtPaymentFee(payment))}`:''}`,'🗑️');},'Delete Payment',true);return true}
let activeWishPurchaseId=null;
function getWishMonthlySavingsRate(){
  let total=0,count=0;
  for(let i=1;i<=3;i++){
...
function updateWishPlanPreview(){
  const wrap=document.getElementById('wish-plan-preview');if(!wrap)return;
  const name=(document.getElementById('w-name')?.value||'').trim();
  const price=roundMoney(parseFloat(document.getElementById('w-price')?.value)||0);
  const priority=document.getElementById('w-priority')?.value||'Want';
  if(price<=0){
    wrap.innerHTML='<div class="wish-plan-empty">Enter a price to preview affordability and savings pace.</div>';
    return;
  }
  const cash=roundMoney(getTrackedAssetBalance());
  const gap=roundMoney(Math.max(price-cash,0));
  const canAfford=gap<=0;
  const monthlyRate=getWishMonthlySavingsRate();
  const monthsAtRate=gap>0&&monthlyRate>0?Math.ceil(gap/monthlyRate):0;
  const eta=monthsAtRate?(()=>{const d=new Date();d.setMonth(d.getMonth()+monthsAtRate);return d.toLocaleDateString('en-PH',{month:'short',year:'numeric'})})():null;
  const plan3=Math.ceil((price/3)/100)*100;
  const plan6=Math.ceil((price/6)/100)*100;
  const tone=canAfford?'good':priority==='Need Soon'?'warn':'';
  const title=name||'Wishlist item';
  const status=canAfford?'Cash can cover this now':monthlyRate>0?`${fmtShort(gap)} more needed · ${monthsAtRate} mo at current pace`:`${fmtShort(gap)} more needed`;
  wrap.innerHTML=`<div class="wish-plan-card ${tone==='good'?'wish-plan-good':tone==='warn'?'wish-plan-warn':''}">
    <div class="wish-plan-head">
      <div><span>PLAN BEFORE BUYING</span><strong>${esc(title)}</strong></div>
      <b>${fmt(price)}</b>
    </div>
    <div class="wish-plan-status">${status}${eta?` · <strong>${eta}</strong>`:''}</div>
    <div class="wish-plan-grid">
      <div><span>3 mo plan</span><strong>${fmtShort(plan3)}/mo</strong></div>
      <div><span>6 mo plan</span><strong>${fmtShort(plan6)}/mo</strong></div>
      <div><span>Available cash</span><strong>${fmtShort(cash)}</strong></div>
    </div>
  </div>`;
}
function addWish(){const name=document.getElementById('w-name').value.trim();const price=parseFloat(document.getElementById('w-price').value)||0;const priority=document.getElementById('w-priority').value;if(!name){showAlert('Enter item name.');return;}wishlist.push({id:nextWishId++,name,price,priority,addedDate:todayStr});document.getElementById('w-name').value='';document.getElementById('w-price').value='';updateWishPlanPreview();closeModal('modal-add-wish');saveData();render()}
...
function convertWishToGoal(id){
  const w=wishlist.find(x=>x.id===id);if(!w)return false;
  const price=roundMoney(Number(w.price||0));
  if(price<=0){showAlert('Enter a valid wishlist price before creating a goal.');return false;}
  const monthly=Math.max(500,Math.ceil((price/6)/100)*100);
  const targetDate=new Date();targetDate.setMonth(targetDate.getMonth()+6);
  showConfirm(`Create a savings goal for "${w.name}"?\n\nTarget: ${fmt(price)}\nSuggested monthly: ${fmt(monthly)}\nThe item will move out of Wishlist into Goals.`,()=>{
    goals.push({id:nextGoalId++,name:w.name,target:price,current:0,monthly,targetDate:toLocal(targetDate)});
    wishlist=wishlist.filter(x=>x.id!==id);
    saveData();
    render();
    showTab('goals');
    showActionToast('Goal created from wishlist',`${w.name} · ${fmt(price)}`,'🎯');
  },'Save First');
  return true;
}
function getWishPurchaseGuidance(price){
  const activeGoals=goals.filter(g=>Number(g.target||0)>Number(g.current||0));
  const remainingTotal=activeGoals.reduce((s,g)=>s+Math.max(Number(g.target||0)-Number(g.current||0),0),0);
  if(!activeGoals.length)return{tone:'good',text:'No active savings goals are competing with this purchase.'};
  const top=[...activeGoals].sort((a,b)=>Math.max(Number(b.target||0)-Number(b.current||0),0)-Math.max(Number(a.target||0)-Number(a.current||0),0))[0];
  const topGap=Math.max(Number(top.target||0)-Number(top.current||0),0);
  const tone=price>topGap*.25?'warn':'info';
  return{tone,text:`Active goals still need ${fmtShort(remainingTotal)}. Biggest gap: ${top.name} (${fmtShort(topGap)} left).`};
}
function updateWishPurchasePreview(){
  const wish=wishlist.find(w=>w.id===activeWishPurchaseId);if(!wish)return;
  const price=roundMoney(Number(wish.price||0));
  const account=document.getElementById('wish-buy-account')?.value||getDefaultAccountKey();
  const wrap=document.getElementById('wish-purchase-impact');if(!wrap)return;
  const guidance=getWishPurchaseGuidance(price);
  const accountName=account?getAccountInfo(account).name:'No account';
  const after=account?roundMoney(Number(nwBalances[account]||0)-price):0;
  wrap.innerHTML=`<div class="wish-purchase-impact ${guidance.tone==='warn'?'wish-purchase-warn':''}">
    <div class="wish-purchase-head"><div><span>ITEM</span><strong>${esc(wish.name)}</strong></div><div><span>PRICE</span><strong>${fmt(price)}</strong></div></div>
    <div class="wish-purchase-row"><span>${esc(accountName)} after purchase</span><strong style="color:${after<0?'var(--red)':'inherit'}">${fmt(after)}</strong></div>
    <div class="wish-purchase-guidance">${guidance.tone==='warn'?'⚠️':'🎯'} ${esc(guidance.text)}</div>
  </div>`;
  renderSpendBalancePreview({wrapId:'wish-buy-balance-preview',buttonId:'wish-buy-save',title:'Purchase balance preview',account,amount:price,submitLabel:'Buy & Log',afterLabel:'After this purchase',okMessage:'Enough balance for this wishlist purchase.',warningMessage:'Not enough balance in this account.'});
}
function buyWish(id){
  ...
  if(!getDefaultAccountKey()){showAlert('Add an account first.');return;}
  activeWishPurchaseId=id;
  buildAccountSelect('wish-buy-account',true,true);
  document.getElementById('wish-buy-account').value=getDefaultAccountKey();
  document.getElementById('wish-buy-date').value=todayStr;
  updateWishPurchasePreview();
  openModal('modal-wish-purchase');
}
function confirmWishPurchase(){
  const w=wishlist.find(x=>x.id===activeWishPurchaseId);if(!w)return;
  const amount=roundMoney(Number(w.price||0));
  const account=document.getElementById('wish-buy-account')?.value||getDefaultAccountKey();
  const date=document.getElementById('wish-buy-date')?.value||todayStr;
  if(amount<=0||!date){showAlert('Enter a valid wishlist price and date.');return;}
  if(!account){showAlert('Choose an account.');return;}
  ...
  entries.unshift(stampRecord({id:nextId++,date,category:'Big Purchases / Goals',amount,note:'Wishlist: '+w.name,account}));
  adjustAccountBalance(account,-amount);
  wishlist=wishlist.filter(x=>x.id!==w.id);
  activeWishPurchaseId=null;
  closeModal('modal-wish-purchase');
  saveData();
  render();
...
function openHistoryPicker(input){
...
function filterHistoryCollections(state=getHistoryState()){
  ...
  if(state.tag){
    expenseData=expenseData.filter(item=>(item.tags||[]).includes(state.tag));
    incomeData=[];
  }
...
function renderHistoryTopbar(state,historySummary){
  ...
  if(wrap)wrap.innerHTML=labels.length?`${labels.map(label=>`<span class="history-active-chip">${esc(label)}</span>`).join('')}<button class="history-active-chip history-clear-chip" type="button" onclick="resetHistoryFilters()">Reset</button>`:'';
}
...
function getHistoryItemOpenAction(item){
  ...
  if(item.isDebtPayment||item.isGoalContribution||item.isDebtPaymentFee||item.isExternalTransfer)return'';
...
function canChangeHistoryItemCategory(item){return item.kind==='expense'&&!item.isDebtPayment&&!item.isGoalContribution&&!item.isDebtPaymentFee&&!item.isExternalTransfer}
function canChangeHistoryItemAccount(item){return !!item.account&&!item.isDebtPaymentFee&&!item.isExternalTransfer}
...
function removeHistoryItem(item){
  ...
  if(item.isDebtPayment||item.isDebtPaymentFee){
    ...
    if(debt)debt.total=roundMoney(Number(debt.total||0)+Number(payment.amount||0));
  ...
  if(item.isExternalTransfer&&item.transferId){
    deleteTransfer(item.transferId);
    return true;
  }
  const entry=entries.find(e=>e.id===item.id);
  if(!entry)return false;
  if(isCCAccount(entry.account)){adjustAccountBalance(entry.account,-Number(entry.amount||0));const _ccA=nwAccounts.find(a=>a.key===entry.account);if(_ccA?.linkedDebtId){const _ld=debts.find(d=>String(d.id)===String(_ccA.linkedDebtId));if(_ld)_ld.total=roundMoney(Math.max(0,Number(_ld.total||0)-Number(entry.amount||0)));}}else{adjustAccountBalance(entry.account,Number(entry.amount||0));}
...
function quickDuplicateHistoryItem(key){
  const [kind,idStr]=key.split(':');const id=parseInt(idStr);
  if(kind==='expense'){
    const e=entries.find(x=>x.id===id);if(!e||e.isDebtPayment||e.isDebtPaymentFee||e.isGoalContribution||e.isExternalTransfer)return;
    const account=e.account||'';const amount=Number(e.amount||0);
    const bs=getSpendValidationState(account,amount);
    if(account&&!bs.hasEnough){showAlert(`Not enough balance in ${getAccountInfo(account).name}. Available: ${fmt(bs.available)}`);return;}
    const dup=stampRecord({...e,id:nextId++,date:todayStr});
    entries.unshift(dup);
    if(isCCAccount(account)){adjustAccountBalance(account,amount);const _ccA=nwAccounts.find(a=>a.key===account);if(_ccA?.linkedDebtId){const _ld=debts.find(d=>String(d.id)===String(_ccA.linkedDebtId));if(_ld)_ld.total=roundMoney(Number(_ld.total||0)+amount);}}else{adjustAccountBalance(account,-amount);}
    saveData();render();showActionToast(`${fmt(amount)} duplicated`,esc(e.category),'🧾');
  }else if(kind==='income'){
    const i=incomes.find(x=>x.id===id);if(!i||i.isSalaryDeposit)return;
    const dup=stampRecord({...i,id:nextIncId++,date:todayStr});
    incomes.unshift(dup);adjustAccountBalance(dup.account,Number(dup.amount||0));
    saveData();render();showActionToast(`${fmt(dup.amount)} duplicated`,esc(i.source),'💵');
  }
}
function renderHistoryCard(item){
  ...
  const canDup=!historyBulkMode&&!item.isDebtPaymentFee&&!item.isDebtPayment&&!item.isGoalContribution&&!item.isExternalTransfer&&(item.kind==='expense'||item.kind==='income');
  const dupBtn=canDup?`<button class="btn-icon tx-dup" onclick="event.stopPropagation();quickDuplicateHistoryItem('${key}')" style="border:none;color:var(--text3);font-size:13px;padding:4px 5px" title="Duplicate to today" aria-label="Duplicate transaction to today">⧉</button>`:'';
  const deleteMarkup=historyBulkMode||item.isDebtPaymentFee?'':`<button class="btn-icon tx-delete" onclick="event.stopPropagation();deleteHistorySingle('${key}')" style="border:none;color:var(--red);font-size:12px" title="Delete transaction" aria-label="Delete transaction">✕</button>`;
  const tagPills=(item.tags&&item.tags.length)?`<div class="tx-tags">${item.tags.map(t=>`<span class="tag-pill" onclick="event.stopPropagation();setHistoryTagFilter('${t.replace(/'/g,"\\'")}')">🏷️ ${esc(t)}</span>`).join('')}</div>`:'';
  return`<div class="tx-item history-tx-item ${historyBulkMode?'history-tx-item-selectable':''} ${selected?'history-tx-item-selected':''}" ${clickAction?`onclick="${clickAction}"`:''}>${selectionMarkup}<div class="tx-icon ${iconInfo.className}">${iconInfo.icon}</div><div class="tx-info"><div class="tx-name">${esc(getHistoryItemLabel(item))}</div><div class="tx-meta">${esc(getHistoryItemMeta(item))}</div>${tagPills}</div><div class="tx-amount" style="color:${item.kind==='income'?'var(--green)':'var(--red)'}">${item.kind==='income'?'+':'-'}${fmt(item.amount)}</div>${dupBtn}${deleteMarkup}</div>`;
}
...
function renderHistoryEmptyState(){
  const hasAnyTransactions=(entries&&entries.length)||(incomes&&incomes.length);
  const hasActiveFilters=getHistoryActiveFilterLabels(getHistoryState(),getHistoryGroupMode()).length>0;
  if(hasAnyTransactions&&hasActiveFilters)return'<div class="empty history-empty-filtered"><div class="empty-icon">📋</div><div class="empty-text">No transactions match these filters</div><div class="history-empty-sub">Try a different date range or clear the active filters.</div><button class="btn btn-sm btn-primary empty-cta" onclick="resetHistoryFilters()">Reset Filters</button></div>';
  return'<div class="empty history-empty-filtered"><div class="empty-icon">📋</div><div class="empty-text">No transactions yet</div><div class="history-empty-sub">Add income or expenses to start tracking your cash flow.</div><button class="btn btn-sm btn-primary empty-cta" onclick="showTab(&quot;add&quot;)">Add Transaction</button></div>';
}
function renderHistoryCardsContent(historyCards,groupMode,totalCount=historyCards.length){
  if(!historyCards.length)return renderHistoryEmptyState();
...
function resetHistoryFilters(){
  ['hist-search','hist-min','hist-max','hist-day','hist-week','hist-from','hist-to','hist-tag'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});
...
function exportCSV(){
  ...
  const rows=[];
  const _catMap=Object.fromEntries(allCats().map(c=>[c.name,c.group||'']));
  if(expenseData.length){
    rows.push(['--- EXPENSES ---']);
    rows.push(['Date','Category','Group','Account','Amount','Tags','Note']);
    rows.push(...expenseData.map(item=>[item.date,toCsvCell(item.category),toCsvCell(_catMap[item.category]||''),toCsvCell(getAccountInfo(item.account).name),item.amount,toCsvCell((item.tags||[]).join('; ')),toCsvCell(item.note||'')]));
  }
  if(incomeData.length){
    rows.push(['--- INCOME ---']);
    rows.push(['Date','Source','Group','Account','Amount','Note']);
    rows.push(...incomeData.map(item=>[item.date,toCsvCell(item.source),'income',toCsvCell(getAccountInfo(item.account).name),item.amount,toCsvCell(item.note||'')]));
  }
...
function openCsvImportModal(){
  csvImportRawText='';
  csvImportDraft=[];
  const file=document.getElementById('csv-import-file');if(file)file.value='';
  const account=document.getElementById('csv-import-account');
  if(account){account.innerHTML=nwAccounts.map(a=>`<option value="${a.key}">${a.icon} ${esc(a.name)}</option>`).join('');account.value=getDefaultAccountKey();}
  const type=document.getElementById('csv-import-type');if(type)type.value='expense';
  const skip=document.getElementById('csv-import-skip-dupes');if(skip)skip.checked=true;
...
function normalizeCsvHeader(value){return String(value||'').trim().toLowerCase().replace(/[^a-z0-9]+/g,'')}
function csvRowObject(headers,row){const obj={};headers.forEach((h,i)=>{obj[normalizeCsvHeader(h)]=String(row[i]??'').trim()});return obj}
function csvValue(obj,names){for(const name of names){const val=obj[normalizeCsvHeader(name)];if(val!==undefined&&val!=='')return val;}return''}
...
function normalizeCsvDate(value){
  ...
  if(slash){let a=Number(slash[1]),b=Number(slash[2]),y=slash[3];if(y.length===2)y='20'+y;const month=a>12?b:a;const day=a>12?a:b;return`${y}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`;}
  const d=new Date(raw);if(!Number.isNaN(d.getTime()))return toLocal(d);
  return'';
}
function findCsvAccount(value,defaultKey){
  const raw=String(value||'').trim().toLowerCase();
  if(raw){const match=nwAccounts.find(a=>a.key.toLowerCase()===raw||String(a.name||'').trim().toLowerCase()===raw);if(match)return match.key;}
  return defaultKey||getDefaultAccountKey();
}
function inferCsvType(obj,section,amount,debit,credit,defaultType){
  const explicit=csvValue(obj,['type','kind','transaction type','direction']).toLowerCase();
  if(section)return section;
  if(/income|credit|deposit|received|money in/.test(explicit))return'income';
  if(/expense|debit|withdraw|payment|purchase|charge|money out/.test(explicit))return'expense';
  if(credit>0)return'income';
  if(debit>0)return'expense';
  if(amount<0)return'expense';
  if(amount>0)return defaultType||'expense';
  return defaultType||'expense';
}
function getCsvFallbackCategory(description){
  const suggested=typeof suggestCategoryFromMerchant==='function'?suggestCategoryFromMerchant(description):'';
  if(suggested&&allCats().some(c=>c.name===suggested))return suggested;
  return allCats().some(c=>c.name==='Miscellaneous / Buffer')?'Miscellaneous / Buffer':(allCats()[0]?.name||'Other');
}
function parseCsvImportRows(text){
  const rows=parseCsvText(text);let headers=null,section='',parsed=[];
  const defaultAccount=document.getElementById('csv-import-account')?.value||getDefaultAccountKey();
  const defaultType=document.getElementById('csv-import-type')?.value||'expense';
  rows.forEach(rawRow=>{
    const first=String(rawRow[0]||'').trim().toLowerCase();
    if(first.includes('expenses')){section='expense';headers=null;return;}
    if(first.includes('income')){section='income';headers=null;return;}
    const normalizedRow=rawRow.map(normalizeCsvHeader);
    const looksHeader=normalizedRow.includes('date')||normalizedRow.some(c=>['amount','debit','credit','description','note','category','source'].includes(c));
    const repeatedHeader=headers&&normalizedRow.includes('date')&&normalizedRow.some(c=>['amount','debit','credit','description','note','category','source'].includes(c));
    if(!headers&&looksHeader){headers=rawRow;return;}
    if(repeatedHeader){headers=rawRow;return;}
    if(!headers)return;
    const obj=csvRowObject(headers,rawRow);
    const date=normalizeCsvDate(csvValue(obj,['date','posted date','transaction date','posting date']));
    const description=csvValue(obj,['description','note','merchant','details','particulars','memo','narration']);
    const debit=parseCsvMoney(csvValue(obj,['debit','withdrawal','outflow','money out','expense']));
    const credit=parseCsvMoney(csvValue(obj,['credit','deposit','inflow','money in','income']));
    const amountRaw=csvValue(obj,['amount','transaction amount','value']);
    const amountVal=parseCsvMoney(amountRaw);
    const type=inferCsvType(obj,section,amountVal,Math.abs(debit),Math.abs(credit),defaultType);
    const amount=roundMoney(Math.abs(credit||debit||amountVal));
    const account=findCsvAccount(csvValue(obj,['account','wallet','bank']),defaultAccount);
    const categoryRaw=csvValue(obj,['category']);
    const sourceRaw=csvValue(obj,['source','income source']);
    const category=type==='expense'?(allCats().some(c=>c.name===categoryRaw)?categoryRaw:getCsvFallbackCategory(description||categoryRaw)):'';
    const source=type==='income'?(sourceRaw||categoryRaw||description||'Imported Income'):'';
    const note=csvValue(obj,['note','description','merchant','details','memo'])||description;
    const duplicate=type==='income'
      ?incomes.some(i=>i.date===date&&Number(i.amount||0)===amount&&(i.source||'')===source&&(i.note||'')===(note||''))
      :entries.some(e=>e.date===date&&Number(e.amount||0)===amount&&(e.category||'')===category&&(e.note||'')===(note||''));
    const invalid=!date||!amount||amount<=0||!account||!['income','expense'].includes(type);
    parsed.push({date,type,amount,account,category,source,note,duplicate,invalid});
  });
  return parsed;
}
function renderCsvImportPreview(){
  ...
  const save=document.getElementById('csv-import-save');
  const skip=document.getElementById('csv-import-skip-dupes')?.checked!==false;
  if(!summary||!preview||!save)return;
  if(!csvImportDraft.length){summary.innerHTML='<div class="csv-import-empty">Choose a CSV file to preview transactions before saving.</div>';preview.innerHTML='';save.disabled=true;save.textContent='Import';return;}
  const valid=csvImportDraft.filter(r=>!r.invalid);
  const dupes=csvImportDraft.filter(r=>r.duplicate&&!r.invalid);
  const ready=valid.filter(r=>!(skip&&r.duplicate));
  const invalid=csvImportDraft.filter(r=>r.invalid);
  const expenseTotal=ready.filter(r=>r.type==='expense').reduce((s,r)=>s+r.amount,0);
  const incomeTotal=ready.filter(r=>r.type==='income').reduce((s,r)=>s+r.amount,0);
  summary.innerHTML=`<div class="csv-import-summary"><div><strong>${ready.length}</strong><span>ready</span></div><div><strong>${dupes.length}</strong><span>duplicates</span></div><div><strong>${invalid.length}</strong><span>invalid</span></div><div><strong>${fmtSigned(incomeTotal-expenseTotal)}</strong><span>net impact</span></div></div>`;
  preview.innerHTML=`<div class="csv-import-list">${csvImportDraft.slice(0,30).map(r=>{const tone=r.invalid?'invalid':r.duplicate?'duplicate':r.type;const label=r.invalid?'Invalid':r.duplicate?'Duplicate':r.type==='income'?'Income':'Expense';return`<div class="csv-import-row ${tone}"><div><div class="csv-import-row-title">${esc(r.type==='income'?r.source:r.category||'Uncategorized')}</div><div class="csv-import-row-meta">${esc(r.date||'No date')} · ${esc(getAccountInfo(r.account).name)}${r.note?` · ${esc(r.note)}`:''}</div></div><div class="csv-import-row-side"><span>${label}</span><strong>${r.type==='income'?'+':'-'}${fmt(r.amount)}</strong></div></div>`}).join('')}</div>${csvImportDraft.length>30?`<div class="csv-import-more">Showing first 30 of ${csvImportDraft.length} rows.</div>`:''}`;
  save.disabled=ready.length===0;
  save.textContent=ready.length?`Import ${ready.length}`:'Import';
}
function reparseCsvImportDraft(){if(!csvImportRawText)return;csvImportDraft=parseCsvImportRows(csvImportRawText);renderCsvImportPreview();}
function handleCsvImportFile(event){
  const file=event.target.files&&event.target.files[0];if(!file)return;
  const reader=new FileReader();
  reader.onload=e=>{csvImportRawText=String(e.target.result||'');csvImportDraft=parseCsvImportRows(csvImportRawText);if(!csvImportDraft.length)showAlert('No importable rows found. Use a CSV with headers such as Date, Description, Amount, Debit, Credit, Category, Source, or Account.','Import CSV');renderCsvImportPreview();};
  reader.readAsText(file);
}
function applyImportedExpense(row){
  const entry=stampRecord({id:nextId++,date:row.date,amount:row.amount,category:row.category,account:row.account,note:row.note||''});
  entries.unshift(entry);
  if(isCCAccount(row.account)){adjustAccountBalance(row.account,row.amount);const cc=nwAccounts.find(a=>a.key===row.account);if(cc?.linkedDebtId){const debt=debts.find(d=>String(d.id)===String(cc.linkedDebtId));if(debt)debt.total=roundMoney(Number(debt.total||0)+row.amount);}}
  else adjustAccountBalance(row.account,-row.amount);
  rememberExpensePattern(entry);
}
function applyImportedIncome(row){
  const income=stampRecord({id:nextIncId++,date:row.date,source:row.source,amount:row.amount,note:row.note||'',account:row.account});
  incomes.unshift(income);adjustAccountBalance(row.account,row.amount);rememberIncomePattern(income);
}
function confirmCsvImport(){
  const skip=document.getElementById('csv-import-skip-dupes')?.checked!==false;
  const ready=csvImportDraft.filter(r=>!r.invalid&&!(skip&&r.duplicate));
  if(!ready.length){showAlert('No valid rows are ready to import.');return;}
  showConfirm(`Import ${ready.length} transaction${ready.length===1?'':'s'}? Account balances will be updated.`,()=>{
    ready.forEach(row=>row.type==='income'?applyImportedIncome(row):applyImportedExpense(row));
    saveData();closeModal('modal-csv-import');showTab('history');render();showActionToast('CSV imported',`${ready.length} transaction${ready.length===1?'':'s'} added.`,'📤');
  },'Import CSV');
}
...
function setSalaryBudgetValue(val){
  ...
  if(newSal<=0){showAlert('Salary must be greater than ₱0. Budget calculations depend on this value.');return;}
  salary=newSal;
  saveData();
  render();
}
...
function renderBudgetSettingsContent({ac,totalBudgeted,needsB,wantsB,savsB,unalloc,catTotals}){
  ...
  mount.innerHTML=`
    <div class="budget-settings-shell">
        ...
        ${(()=>{
          ...
          return sectionOrder.filter(s=>grouped[s]&&grouped[s].length).map(sId=>{
            ...
            const rows=cats.map(c=>{
              ...
              return `<div class="budget-category-row"><div class="budget-category-main"><div class="setting-icon">${c.icon||'&#128230;'}</div><div class="budget-category-copy"><div class="budget-category-top"><div class="setting-name">${esc(c.name)}</div>${isCustom?`<button class="cat-group-pill ${grpClass}" onclick="cycleCatGroup('${safeName}')" title="Tap to change group">${grpLabel}</button>`:`<span class="cat-group-pill ${grpClass}" style="cursor:default">${grpLabel}</span>`}${overBudget?'<span class="budget-over-badge">OVER</span>':''}</div>${showMeta?`<div class="budget-category-meta"><span>${formatBudgetProgress(spent,bgt)}</span>${pctOfSalary>0?`<span>${pctOfSalary}% of salary</span>`:''}</div>`:''} ${spent>0?`<div class="budget-category-progress"><div class="budget-category-progress-fill" style="width:${spentPct}%;background:${overBudget?'var(--red)':'var(--accent)'}"></div></div>`:''}</div></div><div class="budget-category-actions"><div class="budget-input-row"><span>PHP</span><input type="number" class="input setting-input" value="${bgt}" onchange="setBudgetAmount('${safeName}',this.value)"></div>${showChip?`<span class="budget-remaining-chip ${remainingMeta.tone}">${remainingMeta.label}</span>`:''}</div></div>`;
            }).join('');
      ...
      ${(()=>{const co=getCarryoverOverspend();return co>0?`<div class="budget-rollover-notice">↩ <strong>${fmt(co)}</strong> carried over from last month's overspend — reduces this month's available room</div>`:'';})()}
...
function renderDebtFocusCard(data){
  ...
  if(!activeDebts.length){
    el.innerHTML=`<div class="card"><div class="card-header"><span class="card-title">Debt Focus</span><span class="card-badge" style="background:var(--green-soft);color:var(--green)">Cleared</span></div>
      <div style="padding:8px 10px;background:var(--surface2);border-radius:10px;display:flex;align-items:center;gap:8px">
        <span style="font-size:18px">🎉</span>
        <div><div style="font-size:12px;font-weight:700">All tracked debts are fully paid.</div><div style="font-size:11px;color:var(--text2)">${getPaidOffDebts().length} paid-off account${getPaidOffDebts().length!==1?'s':''} in history below.</div></div>
      </div></div>`;
    return;
  }
  ...
  el.innerHTML=`<div class="card"><div class="card-header"><span class="card-title">Debt Focus</span><span class="card-badge" style="background:var(--red-soft);color:var(--red)">${methodLabel}</span></div>
    <div style="display:grid;gap:5px">
      <div style="padding:8px 10px;background:var(--surface2);border-radius:10px;display:flex;align-items:center;justify-content:space-between;gap:8px">
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;font-weight:700">This month</div><div style="font-size:12px;font-weight:700;margin-top:1px">${paidThisMonth} of ${debts.length} debts paid</div></div>
        <div style="font-size:11px;color:var(--text2);text-align:right;white-space:nowrap;flex-shrink:0">Min due<br><strong>${fmtShort(data.monthlyMinimum||0)}</strong></div>
      </div>
      <div style="padding:8px 10px;background:var(--surface2);border-radius:10px;display:flex;align-items:center;gap:8px">
        <div style="min-width:0"><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;font-weight:700">Focus</div><div style="font-size:12px;font-weight:700;margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${data.targetDebt?esc(data.targetDebt.name):'Pay minimums on all debts'}</div><div style="font-size:11px;color:var(--text2)">${data.extraPayment>0&&data.targetDebt?`Extra: ${fmtShort(data.extraPayment)}`:'No safe extra payment right now'}</div></div>
      </div>
      ${data.riskText?`<div style="padding:6px 10px;background:var(--red-soft);border-radius:10px;font-size:11px;color:var(--red);font-weight:600">⚠️ ${data.riskText}</div>`:''}
    </div></div>`;
}
...
function applyWidgetOrder(){
  ...
  // Keep show-more and widget panel at the end, preserve compact-forecast position (handled by JS)
  ...
  const panel=document.getElementById('widget-settings-panel');if(panel)section.appendChild(panel);
}
function openWidgetSettings(){
  const panel=document.getElementById('widget-settings-panel');if(!panel)return;
  const isOpen=panel.style.display!=='none';
  panel.style.display=isOpen?'none':'block';
  if(!isOpen)renderWidgetSettingsPanel();
}
...
function renderWidgetSettingsPanel(){
  const panel=document.getElementById('widget-settings-panel');if(!panel)return;
  const config=loadDashboardWidgetConfig();
  panel.innerHTML=`<div class="card" style="margin:0"><div class="card-header"><span class="card-title">⚙ Dashboard Layout</span><button class="btn btn-sm btn-ghost" onclick="openWidgetSettings()">✕ Close</button></div><div style="padding:0 16px 4px;font-size:12px;color:var(--text3)">Show, hide, and reorder your dashboard cards.</div><div style="padding:0 16px 16px">${config.map((w,i)=>`<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--border)"><span style="flex:1;font-size:13px;color:${w.visible?'var(--text)':'var(--text3)'}">${w.label}</span><button class="btn btn-sm btn-ghost" style="padding:2px 7px" onclick="moveWidgetUp('${w.id}')"${i===0?' disabled':''}>↑</button><button class="btn btn-sm btn-ghost" style="padding:2px 7px" onclick="moveWidgetDown('${w.id}')"${i===config.length-1?' disabled':''}>↓</button><button class="btn btn-sm ${w.visible?'btn-primary':'btn-ghost'}" style="padding:2px 8px;min-width:38px" onclick="toggleWidgetVisible('${w.id}')">${w.visible?'On':'Off'}</button></div>`).join('')}<button class="btn btn-ghost btn-sm" style="margin-top:12px;width:100%" onclick="resetWidgetOrder()">Reset to Default</button></div></div>`;
}
...
function renderGettingStartedCard(){
  const el=document.getElementById('gs-card');if(!el)return;
  if(localStorage.getItem('ft_show_gs')!=='1'){el.innerHTML='';return;}
  el.innerHTML=`<div class="card gs-card-inner"><div class="gs-card-dismiss" onclick="dismissGettingStartedCard()" aria-label="Dismiss">✕</div><div class="gs-card-icon">👋</div><div class="gs-card-title">Welcome to FinTrack!</div><div class="gs-card-body">Here are a few things to get you started:</div><ul class="gs-card-list"><li>💰 Set your <strong>salary</strong> in the <em>More</em> tab under Budget Settings</li><li>🏦 Add your <strong>accounts</strong> in the <em>More</em> tab</li><li>📝 Log your first <strong>expense</strong> using the <em>Add</em> tab</li><li>🎯 Create a <strong>savings goal</strong> in the <em>Goals</em> tab</li></ul><button class="btn btn-primary gs-card-btn" onclick="dismissGettingStartedCard()">Got it!</button></div>`;
}
function render(){
  ...
  try{ renderSpendForecastCard(); }catch(e){}
  ...
  document.getElementById('health-mini').innerHTML=`<div class="card" style="cursor:pointer" onclick="showTab('goals')"><div style="display:flex;align-items:center;gap:16px"><svg width="56" height="56" viewBox="0 0 100 100"><circle cx="50" cy="50" r="42" class="score-bg" stroke-width="7"/><circle cx="50" cy="50" r="42" fill="none" stroke="${hs.color}" stroke-width="7" stroke-linecap="round" stroke-dasharray="${circ}" stroke-dashoffset="${offset}" class="score-fill"/><line x1="71.7" y1="79.9" x2="77.6" y2="88.0" stroke="#f59e0b" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/><line x1="14.8" y1="61.4" x2="5.3" y2="64.5" stroke="#10b981" stroke-width="2.5" stroke-linecap="round" opacity="0.85"/><text x="50" y="50" class="score-text" font-size="22">${hs.total}</text></svg><div><div style="font-weight:700;font-size:14px;color:${hs.color}">${hs.grade}</div><div style="font-size:12px;color:var(--text3)">Financial Health Score</div><div style="font-size:10px;color:var(--text3);margin-top:2px"><span style="color:#10b981">●</span> Good ≥70 &nbsp;<span style="color:#f59e0b">●</span> Fair ≥40</div></div></div></div>`;
  ...
  // Budget bars (full)
  document.getElementById('budget-bars').innerHTML=budgetProgressItems.length?budgetProgressItems.map(c=>{const pct=c.budget>0?Math.min(c.pct,100):0;const color=c.over?'var(--red)':pct>=80?'var(--amber)':'var(--green)';const rolloverBadge=c.rollover>0?`<span class="rollover-badge">+${fmtShort(c.rollover)} rollover</span>`:'';const canRollover=c.type==='variable';const rolloverOn=!!rolloverSettings[c.name];const rolloverBtn=canRollover?`<button class="budget-edit-btn" onclick="toggleCategoryRollover('${esc(c.name)}')" title="${rolloverOn?'Disable rollover':'Enable rollover — unspent budget carries to next month'}" style="color:${rolloverOn?'var(--blue)':'var(--text3)'}">${rolloverOn?'↩':'↩'}</button>`:'';return`<div class="progress"><div class="progress-header"><span class="progress-label">${c.icon||'📦'} ${esc(c.name.length>22?c.name.substring(0,22)+'…':c.name)}${rolloverBadge}</span><div class="budget-val-row"><span class="progress-value" style="color:${color}">${formatBudgetProgress(c.spent,c.budget)}</span>${rolloverBtn}<button class="budget-edit-btn" onclick="openBudgetInlineEdit(this,'${esc(c.name)}',${c.budget})" title="Edit budget">✏</button></div></div><div class="progress-track"><div class="progress-fill" style="width:${pct}%;background:${color}"></div></div></div>`}).join(''):'<div class="empty"><div class="empty-icon">📒</div><div class="empty-text">Set a budget or log spending to populate this list</div></div>';
  ...
  document.getElementById('month-chart').innerHTML=`<div class="bar-chart">${months.map((m,i)=>{const isCur=i===5;const pct=Math.max(m.total/maxM*100,m.total>0?2:0);const bg=isCur?'linear-gradient(90deg,var(--accent),#818cf8)':m.total>0?'var(--text3)':'transparent';const opacity=isCur?1:m.total>0?.45:1;const amtStyle=isCur?'color:var(--accent);font-weight:800':'';return`<div class="bar-row"><div class="bar-label" style="${isCur?'color:var(--accent);font-weight:800':''}">${m.label}</div><div class="bar-track"><div class="bar-fill" style="width:${pct}%;background:${bg};opacity:${opacity}"></div></div><div class="bar-amount" style="${amtStyle}">${m.total>0?fmtShort(m.total):'—'}</div></div>`}).join('')}</div>`;
  ...
  const histTagEl=document.getElementById('hist-tag');const activeTag=histTagEl?.value||'';
  const histTagChips=document.getElementById('hist-tag-chips');
  if(histTagChips){const allTags=[...new Set(entries.flatMap(e=>e.tags||[]))].sort();histTagChips.innerHTML=(activeTag?`<button class="quick-chip" style="border-color:var(--accent);color:var(--accent);background:var(--accent-soft)" onclick="clearHistoryTagFilter()">🏷️ ${esc(activeTag)} ✕</button>`:'')+(allTags.filter(t=>t!==activeTag).map(t=>`<button class="quick-chip" onclick="setHistoryTagFilter('${t.replace(/'/g,"\\'")}')">${esc(t)}</button>`).join('')||(!activeTag?'<span style="font-size:12px;color:var(--text3)">No tags yet — add tags when logging expenses</span>':''));}
  ...
  hcEl.innerHTML=renderHistoryCardsContent(visibleHistoryCards,histGroupMode,historyCards.length);syncHistoryDrawerState();
  try{ renderHistoryAnalytics(filtEnt,filtInc); }catch(e){}
  ...
  document.getElementById('goals-list').innerHTML=(()=>{
    if(!goals.length)return'<div class="empty"><div class="empty-icon">🎯</div><div class="empty-text">No goals yet — set one to start tracking your savings targets</div><button class="btn btn-sm btn-primary empty-cta" onclick="openModal(\'modal-add-goal\')">+ Set a Goal</button></div>';
  ...
  const _wishTotalCash=getTrackedAssetBalance();
  // Monthly savings rate: salary minus average spending over last 3 months
  const _wishMonthlySavings=getWishMonthlySavingsRate();
  const wishPriorityOrder=['Need Soon','Want','Medium','Low'];
  const wishGroups=wishPriorityOrder.map(priority=>({priority,items:wishlist.filter(w=>w.priority===priority)})).filter(g=>g.items.length);
  document.getElementById('wishlist').innerHTML=wishlist.length?wishGroups.map(group=>`<div class="wish-priority-group"><div class="wish-priority-head"><span>${esc(group.priority)}</span><strong>${group.items.length}</strong></div>${group.items.map(w=>{
    ...
    return`<div class="wish-card" style="display:block;border-left:3px solid ${bc}"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:5px"><div style="font-weight:700;font-size:14px">${esc(w.name)}</div><span style="font-size:11px;font-weight:700;color:${bc};background:${bbg};padding:3px 8px;border-radius:999px;white-space:nowrap;flex-shrink:0">${w.priority}</span></div><div style="font-size:11px;color:var(--text3);margin-bottom:10px">Added ${addedFmt} · ${days===0?'today':days+'d ago'}</div><div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:${affordHint?'8px':'12px'}"><div style="font-size:20px;font-weight:800;color:var(--amber)">${fmt(price)}</div>${affordBadge}</div>${affordHint?affordHint+'<div style="height:12px"></div>':''}<div class="wish-actions"><button class="btn btn-sm btn-primary" onclick="event.stopPropagation();buyWish(${w.id})">🛒 Buy Now</button><button class="btn btn-sm btn-ghost" onclick="event.stopPropagation();convertWishToGoal(${w.id})">🎯 Save First</button><button class="btn btn-sm btn-ghost" onclick="event.stopPropagation();deleteWish(${w.id})">Remove</button></div></div>`;
  }).join('')}</div>`).join(''):'<div class="empty"><div class="empty-icon">🛒</div><div class="empty-text">Nothing on your wishlist yet — add things you\'re saving up for</div><button class="btn btn-sm btn-primary empty-cta" onclick="openModal(\'modal-add-wish\')">+ Add Item</button></div>';
  ...
  try{ renderSplitsCard(); }catch(e){}

  // === MORE ===
  const {assetAccounts:_assetAccts,creditCardAccounts:_ccAccts,assetTotal:nwT,creditCardTotal:_nwCC,unlinkedDebtTotal:_unlinkedDebt,debtTotal:_nwDisplayDebt,net:netW}=getNetWorthSummary();
  const _assetRows=_assetAccts.length?'<div class="nw-section-label">Bank / E-Wallet</div><div class="nw-bank-grid" id="nw-grid-assets">'+_assetAccts.map(a=>{const bal=Number(nwBalances[a.key]||0);const brand=getBankBrandVisual(a.name,a.icon);return`<div class="nw-bank-card" style="background:${brand.bg}"><div class="nw-card-deco nw-card-deco-1"></div><div class="nw-card-deco nw-card-deco-2"></div><input type="number" id="nw-${a.key}" value="${bal}" style="display:none"><div class="nw-card-top"><div class="nw-bank-logo">${nwBrandLogoHtml(brand,a.icon)}</div><button class="nw-bank-edit" onclick="openNetWorthEdit('${a.key}')" title="Account options" aria-label="Account options">⋯</button></div><div class="nw-bank-balance">${fmt(bal)}</div><div class="nw-bank-name">${esc(a.name)}</div><div class="nw-bank-badge">🏦 Asset</div></div>`;}).join('')+'</div>':'';
  const _ccRows=_ccAccts.length?'<div class="nw-section-label">Credit Cards</div><div class="nw-bank-grid" id="nw-grid-cc">'+_ccAccts.map(a=>{const bal=Number(nwBalances[a.key]||0);const limit=Number(a.creditLimit||0);const available=limit>0?Math.max(limit-bal,0):null;const usedPct=limit>0?Math.min(bal/limit*100,100):0;const linkedDebt=a.linkedDebtId?debts.find(d=>String(d.id)===String(a.linkedDebtId)):null;const brand=getBankBrandVisual(a.name,a.icon);const _uc=usedPct>=80?'#fca5a5':usedPct>=50?'#fde68a':'rgba(255,255,255,.78)';return`<div class="nw-bank-card" style="background:${brand.bg}"><div class="nw-card-deco nw-card-deco-1"></div><div class="nw-card-deco nw-card-deco-2"></div><div class="nw-card-top"><div class="nw-bank-logo">${nwBrandLogoHtml(brand,a.icon)}</div><button class="nw-bank-edit" onclick="openNetWorthEdit('${a.key}')" title="Account options" aria-label="Account options">⋯</button></div><div class="nw-bank-balance">${fmt(bal)}</div><div class="nw-bank-name">${esc(a.name)}</div><div class="nw-bank-badge">💳 Credit${linkedDebt?' · linked':''}</div>${limit>0?`<div class="nw-bank-cc-bar"><div class="nw-bank-cc-bar-fill" style="width:${usedPct}%;background:${_uc}"></div></div><div class="nw-bank-cc-avail">${fmtShort(available??0)} available</div>`:''}</div>`;}).join('')+'</div>':'';
  ...
  _makeDragScroll(document.getElementById('nw-grid-assets'));
  _makeDragScroll(document.getElementById('nw-grid-cc'));
  applyNwViewMode();
  const _nwBarTotal=nwT+_nwDisplayDebt;const _assetPct=_nwBarTotal>0?Math.max(nwT/_nwBarTotal*100,0):50;const _debtPct=_nwBarTotal>0?Math.max(_nwDisplayDebt/_nwBarTotal*100,0):50;const _nwMsg=netW>=0?`You own ${fmtShort(netW)} more than you owe`:`Debts exceed assets by ${fmtShort(Math.abs(netW))}`;
  document.getElementById('nw-total').innerHTML=`<div style="margin-top:14px"><div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text3);margin-bottom:5px"><span>Assets <strong style="color:var(--blue)">${fmtShort(nwT)}</strong></span><span>Debts <strong style="color:var(--red)">${fmtShort(_nwDisplayDebt)}</strong></span></div><div style="height:5px;background:var(--border);border-radius:999px;overflow:hidden;display:flex"><div style="width:${_assetPct}%;background:var(--blue);opacity:.7;transition:width .4s;flex-shrink:0;min-width:${nwT>0?4:0}px"></div><div style="width:${_debtPct}%;background:var(--red);opacity:.7;flex-shrink:0;min-width:${_nwDisplayDebt>0?4:0}px"></div></div></div><div style="text-align:center;padding:6px 0 2px"><div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px">Net Worth</div><div style="font-size:24px;font-weight:800;color:${netW>=0?'var(--green)':'var(--red)'};margin:4px 0">${fmt(netW)}</div><div style="font-size:11px;color:var(--text3)">${_nwMsg}</div></div></div>`;
...
toggleRecurringType();
migrateExternalTransferHistoryEntries();
render();
