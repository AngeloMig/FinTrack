...
function getUpcomingSalarySplits(){normalizePaySchedule();const monthKey=currentMonthKey();const todayDay=now.getDate();const received=paySchedule.received||{};return (paySchedule.splits||[]).filter(split=>split.day>todayDay&&!received[getSalaryReceiptKey(monthKey,split.day)]).sort((a,b)=>a.day-b.day)}
function openSalaryReceiptModal(day,isEarly=false){normalizePaySchedule();const monthKey=currentMonthKey();const split=(paySchedule.splits||[]).find(s=>parseInt(s.day)===parseInt(day));if(!split)return;const receiptKey=getSalaryReceiptKey(monthKey,split.day);if(getSalaryReceiptRecord(receiptKey)){showAlert(`Salary for the ${formatOrdinalDay(split.day)} is already recorded this month.`);render();return;}activeSalaryReceiptKey=receiptKey;document.getElementById('salary-receive-subtext').textContent=isEarly?`Salary for the ${formatOrdinalDay(split.day)} arrived early. Confirm this deposit now.`:`Confirm your salary deposit for the ${formatOrdinalDay(split.day)}.`;document.getElementById('sr-amount').value=split.amount||'';document.getElementById('sr-date').value=todayStr;buildAccountSelect('sr-account',false);document.getElementById('sr-account').value=split.account||getDefaultAccountKey();document.getElementById('sr-note').value='';openModal('modal-salary-receive')}
function saveSalaryReceipt(){normalizePaySchedule();const amount=parseFloat(document.getElementById('sr-amount').value)||0;const date=document.getElementById('sr-date').value||todayStr;const account=document.getElementById('sr-account').value||getDefaultAccountKey();const note=(document.getElementById('sr-note').value||'').trim();if(amount<=0){showAlert('Enter a valid salary amount.');return;}if(!activeSalaryReceiptKey){showAlert('Open the salary prompt again before confirming this deposit.');return;}if(getSalaryReceiptRecord(activeSalaryReceiptKey)){closeModal('modal-salary-receive');showAlert('This salary is already recorded. Delete the existing salary deposit first if you need to log it again.');render();return;}const split=(paySchedule.splits||[]).find(s=>getSalaryReceiptKey(currentMonthKey(),s.day)===activeSalaryReceiptKey);if(!split){showAlert('Salary schedule not found.');return;}const incomeNote=note||`Scheduled salary · day ${split.day}`;incomes.unshift(stampRecord({id:nextIncId++,date,source:'Salary',amount,note:incomeNote,account,isSalaryDeposit:true,salaryReceiptKey:activeSalaryReceiptKey,salaryScheduledDay:split.day}));adjustAccountBalance(account,amount);paySchedule.received=paySchedule.received||{};paySchedule.received[activeSalaryReceiptKey]={amount,date,account,note:incomeNote,scheduledDay:split.day,createdAt:new Date().toISOString()};activeSalaryReceiptKey='';closeModal('modal-salary-receive');saveData();render();showActionToast(`${fmt(amount)} salary received`,`${getAccountInfo(account).name}`,'💼');showTab('dashboard')}
function renderSalaryPromptCard(){const wrap=document.getElementById('salary-prompt-card');if(!wrap)return;const pending=getPendingSalarySplits();const upcoming=getUpcomingSalarySplits();if(!pending.length&&!upcoming.length){wrap.innerHTML='';return}if(pending.length){const nextSplit=pending[0];const earlySplit=upcoming[0]||null;const accountInfo=getAccountInfo(nextSplit.account);wrap.innerHTML=`<div class="card salary-prompt-card salary-prompt-ready"><div class="salary-prompt-main"><div class="salary-prompt-icon">💼</div><div class="salary-prompt-copy"><div class="salary-prompt-kicker">Salary Ready</div><div class="salary-prompt-title-row"><div class="salary-prompt-title-block"><div class="salary-prompt-title">Receive Salary</div><div class="salary-prompt-sub">Payday for the ${formatOrdinalDay(nextSplit.day)} is ready to confirm.</div></div><span class="salary-prompt-amount">${fmt(nextSplit.amount)}</span></div><div class="salary-prompt-meta"><span class="salary-prompt-pill">Default account <strong>${esc(accountInfo.name)}</strong></span>${earlySplit?`<button type="button" class="salary-prompt-subtle-action" onclick="openSalaryReceiptModal(${earlySplit.day},true)">Received ${formatOrdinalDay(earlySplit.day)} early?</button>`:''}</div><div class="salary-prompt-actions"><button class="btn btn-primary salary-prompt-primary" onclick="openSalaryReceiptModal(${nextSplit.day})">💵 Receive Salary</button></div></div></div></div>`;return}const nextSplit=upcoming[0];const accountInfo=getAccountInfo(nextSplit.account);wrap.innerHTML=`<div class="card salary-prompt-card salary-prompt-upcoming"><div class="salary-prompt-main"><div class="salary-prompt-icon">📅</div><div class="salary-prompt-copy"><div class="salary-prompt-kicker">Next Salary</div><div class="salary-prompt-title-row"><div class="salary-prompt-title-block"><div class="salary-prompt-title">Scheduled for the ${formatOrdinalDay(nextSplit.day)}</div><div class="salary-prompt-sub">If it arrives early, you can log it here without waiting for payday.</div></div><span class="salary-prompt-amount">${fmt(nextSplit.amount)}</span></div><div class="salary-prompt-meta"><span class="salary-prompt-pill">Default account <strong>${esc(accountInfo.name)}</strong></span></div><div class="salary-prompt-actions"><button type="button" class="salary-prompt-subtle-action" onclick="openSalaryReceiptModal(${nextSplit.day},true)">Received early?</button></div></div></div></div>`}
...
function render(){
  ...
  // === MORE ===
  const nwT=nwAccounts.reduce((s,a)=>s+(Number(nwBalances[a.key]||0)),0);const netW=nwT-totalDebt;
  document.getElementById('nw-inputs').innerHTML=nwAccounts.length?nwAccounts.map(a=>{const bal=Number(nwBalances[a.key]||0);const pct=nwT>0?Math.round(bal/nwT*100):0;return`<div class="nw-account"><div class="nw-name">${a.icon} ${a.name}${bal>0&&nwT>0?`<span style="font-size:10px;color:var(--text3);font-weight:400;margin-left:4px">${pct}%</span>`:''}</div><div class="nw-row-actions"><span style="color:var(--text3)">₱</span><input type="number" class="input" id="nw-${a.key}" value="${bal}" style="width:96px;text-align:right"><button class="btn-icon" onclick="openNetWorthEdit('${a.key}')" title="Edit">✏️</button></div></div>`}).join(''):'<div class="empty"><div class="empty-icon">🏦</div><div class="empty-text">Add your first account</div></div>';
  const _nwBarTotal=nwT+totalDebt;const _assetPct=_nwBarTotal>0?Math.max(nwT/_nwBarTotal*100,0):50;const _debtPct=_nwBarTotal>0?Math.max(totalDebt/_nwBarTotal*100,0):50;const _nwMsg=netW>=0?`You own ${fmtShort(netW)} more than you owe`:`Debts exceed assets by ${fmtShort(Math.abs(netW))}`;
  document.getElementById('nw-total').innerHTML=`<div style="margin-top:16px"><div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text3);margin-bottom:5px"><span>Assets <strong style="color:var(--blue)">${fmtShort(nwT)}</strong></span><span>Debts <strong style="color:var(--red)">${fmtShort(totalDebt)}</strong></span></div><div style="height:8px;background:var(--border);border-radius:4px;overflow:hidden;display:flex"><div style="width:${_assetPct}%;background:var(--blue);transition:width .4s;flex-shrink:0;min-width:${nwT>0?4:0}px"></div><div style="width:${_debtPct}%;background:var(--red);flex-shrink:0;min-width:${totalDebt>0?4:0}px"></div></div></div><div style="text-align:center"><div style="font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:.4px">Net Worth</div><div style="font-size:28px;font-weight:800;color:${netW>=0?'var(--green)':'var(--red)'};margin:4px 0">${fmt(netW)}</div><div style="font-size:12px;color:var(--text3)">${_nwMsg}</div></div></div>`;
  ...
  document.getElementById('year-top').innerHTML=topC.length?`<div style="font-size:13px;font-weight:700;margin-bottom:8px">Top Categories</div><div class="bar-chart">${topC.map((t,i)=>{const pct=tcTotal>0?Math.round(t[1]/tcTotal*100):0;return`<div class="bar-row"><div class="bar-label" style="width:60px;font-size:10px">${esc(t[0].substring(0,12))}</div><div class="bar-track"><div class="bar-fill" style="width:${Math.max(t[1]/tcM*100,5)}%;background:${CHART_COLORS[i]}">${t[1]/tcM>.2?fmtShort(t[1]):''}</div></div><div class="bar-amount" style="min-width:64px;text-align:right">${fmtShort(t[1])} <span style="font-size:10px;color:var(--text3)">${pct}%</span></div></div>`}).join('')}</div>`:'';
