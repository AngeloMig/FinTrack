...
function openNetWorthAdd(){editingNetWorthKey=null;document.getElementById('nw-modal-title').textContent='Add Account';const kicker=document.getElementById('nw-modal-kicker');if(kicker)kicker.textContent='Create Net Worth Account';document.getElementById('nw-acc-icon').value='🏦';document.getElementById('nw-acc-name').value='';document.getElementById('nw-acc-balance').value='';document.getElementById('nw-cc-limit').value='';document.getElementById('nw-delete-btn').style.display='none';document.getElementById('nw-transfer-btn').style.display='none';setNwAccountType('asset');populateNwDebtSelect();const pw=document.getElementById('nw-bank-presets');if(pw){pw.style.display='';pw.innerHTML=NW_BANK_PRESETS.map(p=>{const brand=getBankBrandVisual(p.name,p.icon);return`<button type="button" class="nw-preset-chip" style="--chip-bg:${brand.bg}" onclick="applyNwPreset(this,'${p.name}','${p.icon}','${p.type}')"><span class="nw-preset-logo">${nwBrandLogoHtml(brand,p.icon)}</span><span class="nw-preset-label">${p.name}</span></button>`;}).join('');}renderNwModalPreview();openModal('modal-nw-account')}
...
function renderNwModalPreview(){const mount=document.getElementById('nw-modal-preview');if(!mount)return;const icon=document.getElementById('nw-acc-icon')?.value||'🏦';const isCreditCard=document.getElementById('nw-type-credit')?.classList.contains('active');const accountType=isCreditCard?'credit_card':'asset';const rawName=(document.getElementById('nw-acc-name')?.value||'').trim();const linkedDebtId=isCreditCard?(document.getElementById('nw-linked-debt')?.value||''):'';const creditLimit=isCreditCard?(parseFloat(document.getElementById('nw-cc-limit')?.value)||0):0;const linkedDebt=linkedDebtId?debts.find(d=>String(d.id)===String(linkedDebtId)):null;const balance=isCreditCard&&linkedDebt?Number(linkedDebt.total||0):(parseFloat(document.getElementById('nw-acc-balance')?.value)||0);const brand=getBankBrandVisual(rawName,icon);const typeLabel=accountType==='credit_card'?'Credit Card':'Asset Account';const metaLine=accountType==='credit_card'?(creditLimit>0?`${fmtShort(Math.max(creditLimit-balance,0))} left of ${fmtShort(creditLimit)}`:linkedDebt?'Synced from linked debt balance':'Manual credit balance'):(rawName?'Included in assets and net worth':'Set a starting balance for this account');const chips=[accountType==='credit_card'&&linkedDebt?`<span class="nw-modal-preview-chip">Linked to ${esc(linkedDebt.name)}</span>`:'',accountType==='credit_card'&&!linkedDebt?'<span class="nw-modal-preview-chip">Manual balance</span>':'',creditLimit>0?`<span class="nw-modal-preview-chip">Limit ${fmtShort(creditLimit)}</span>`:'',accountType!=='credit_card'&&balance>0?'<span class="nw-modal-preview-chip">Ready to track</span>':''].filter(Boolean).join('');mount.innerHTML=`<div class="nw-modal-preview-shell"><div class="nw-modal-preview-label">Preview</div><div class="nw-modal-preview-card" style="--preview-bg:${brand.bg}"><div class="nw-modal-preview-top"><div class="nw-modal-preview-logo">${nwBrandLogoHtml(brand,icon)}</div><span class="nw-modal-preview-pill">${typeLabel}</span></div><div class="nw-modal-preview-balance">${fmt(balance)}</div><div class="nw-modal-preview-name">${esc(rawName||'New account')}</div><div class="nw-modal-preview-meta-line">${esc(metaLine)}</div>${chips?`<div class="nw-modal-preview-meta">${chips}</div>`:''}</div></div>`;}
