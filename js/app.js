...
function renderNwModalPreview(){const mount=document.getElementById('nw-modal-preview');if(!mount)return;const icon=document.getElementById('nw-acc-icon')?.value||'🏦';const isCreditCard=document.getElementById('nw-type-credit')?.classList.contains('active');const accountType=isCreditCard?'credit_card':'asset';const rawName=(document.getElementById('nw-acc-name')?.value||'').trim();const linkedDebtId=isCreditCard?(document.getElementById('nw-linked-debt')?.value||''):'';const creditLimit=isCreditCard?(parseFloat(document.getElementById('nw-cc-limit')?.value)||0):0;const linkedDebt=linkedDebtId?debts.find(d=>String(d.id)===String(linkedDebtId)):null;const balance=isCreditCard&&linkedDebt?Number(linkedDebt.total||0):(parseFloat(document.getElementById('nw-acc-balance')?.value)||0);const brand=getBankBrandVisual(rawName,icon);const available=creditLimit>0?Math.max(creditLimit-balance,0):null;const usedPct=creditLimit>0?Math.min(balance/creditLimit*100,100):0;const metaLine=!isCreditCard?(rawName?'Included in assets and net worth':'Set a starting balance for this account'):'';const chips=[isCreditCard&&linkedDebt?`<span class="nw-modal-preview-chip">Linked to ${esc(linkedDebt.name)}</span>`:'',isCreditCard&&!linkedDebt?'<span class="nw-modal-preview-chip">Manual balance</span>':'',creditLimit>0?`<span class="nw-modal-preview-chip">Limit ${fmtShort(creditLimit)}</span>`:'',!isCreditCard&&balance>0?'<span class="nw-modal-preview-chip">Ready to track</span>':''].filter(Boolean).join('');const chipEl=isCreditCard?'<div class="nw-card-chip"></div>':'';const balanceLabelEl=isCreditCard?'<div class="nw-modal-preview-balance-label">Outstanding Balance</div>':'';const utilBarEl=isCreditCard&&creditLimit>0?`<div class="nw-card-util-wrap"><div class="nw-card-util-bar"><div class="nw-card-util-fill" style="width:${usedPct}%"></div></div><div class="nw-card-util-labels"><span>${fmtShort(balance)} used</span><span>${fmtShort(available??0)} left of ${fmtShort(creditLimit)}</span></div></div>`:'';const pillLabel=isCreditCard?'💳 Credit Card':'🏦 Asset Account';mount.innerHTML=`<div class="nw-modal-preview-shell"><div class="nw-modal-preview-label">Preview</div><div class="nw-modal-preview-card" style="--preview-bg:${brand.bg}"><div class="nw-card-deco nw-card-deco-1"></div><div class="nw-card-deco nw-card-deco-2"></div><div class="nw-modal-preview-top"><div class="nw-modal-preview-logo">${nwBrandLogoHtml(brand,icon)}</div><span class="nw-modal-preview-pill">${pillLabel}</span></div>${chipEl}${balanceLabelEl}<div class="nw-modal-preview-balance">${fmt(balance)}</div><div class="nw-modal-preview-name">${esc(rawName||'New account')}</div>${metaLine?`<div class="nw-modal-preview-meta-line">${esc(metaLine)}</div>`:''}${utilBarEl}${chips?`<div class="nw-modal-preview-meta">${chips}</div>`:''}</div></div>`;}
...
function render(){
  ...
  const _assetRows='<div class="nw-bank-grid">'+_assetAccts.map(a=>{const bal=Number(nwBalances[a.key]||0);const brand=getBankBrandVisual(a.name,a.icon);return`<div class="nw-bank-card" style="background:${brand.bg}"><input type="number" id="nw-${a.key}" value="${bal}" style="display:none"><div class="nw-card-top"><div class="nw-bank-logo">${nwBrandLogoHtml(brand,a.icon)}</div><button class="nw-bank-edit" onclick="openNetWorthEdit('${a.key}')" title="Account options" aria-label="Account options">⋯</button></div><div class="nw-bank-balance">${fmt(bal)}</div><div class="nw-bank-name">${esc(a.name)}</div><div class="nw-bank-type">Asset</div></div>`;}).join('')+'</div>';
  const _ccRows=_ccAccts.length?'<div class="nw-section-label">Credit Cards</div><div class="nw-bank-grid">'+_ccAccts.map(a=>{const bal=Number(nwBalances[a.key]||0);const limit=Number(a.creditLimit||0);const available=limit>0?Math.max(limit-bal,0):null;const usedPct=limit>0?Math.min(bal/limit*100,100):0;const linkedDebt=a.linkedDebtId?debts.find(d=>String(d.id)===String(a.linkedDebtId)):null;const brand=getBankBrandVisual(a.name,a.icon);return`<div class="nw-bank-card" style="background:${brand.bg}"><div class="nw-card-top"><div class="nw-bank-logo">${nwBrandLogoHtml(brand,a.icon)}</div><button class="nw-bank-edit" onclick="openNetWorthEdit('${a.key}')" title="Account options" aria-label="Account options">⋯</button></div><div class="nw-bank-balance">${fmt(bal)}</div><div class="nw-bank-name">${esc(a.name)}</div><div class="nw-bank-type">Credit${linkedDebt?' · linked':''}${limit>0?` · ${fmtShort(available??0)} left`:''}</div>${limit>0?`<div class="nw-bank-cc-bar"><div class="nw-bank-cc-bar-fill" style="width:${usedPct}%"></div></div>`:''}</div>`;}).join('')+'</div>':'';
