...
function renderSalaryPromptCard(){const wrap=document.getElementById('salary-prompt-card');if(!wrap)return;const pending=getPendingSalarySplits();const upcoming=getUpcomingSalarySplits();if(!pending.length&&!upcoming.length){wrap.innerHTML='';return}let html='';if(pending.length){const s=pending[0];const acct=getAccountInfo(s.account);const ord=s.day===1?'st':s.day===2?'nd':s.day===3?'rd':'th';html+=`<div class="card"><div class="card-header"><div><div class="card-title">Receive Salary</div><div class="card-subtitle">Payday for the ${s.day}${ord} is ready to confirm</div></div><span class="card-badge" style="background:var(--green-soft);color:var(--green)">${fmt(s.amount)}</span></div><div style="font-size:13px;color:var(--text2);line-height:1.7;margin-bottom:12px">Default account: <strong>${esc(acct.name)}</strong>. You can change it before saving.</div><button class="btn btn-primary" onclick="openSalaryReceiptModal(${s.day})">💵 Receive Salary</button></div>`;}if(upcoming.length){const s=upcoming[0];const acct=getAccountInfo(s.account);const ord=s.day===1?'st':s.day===2?'nd':s.day===3?'rd':'th';html+=`<div class="card" style="margin-top:10px"><div class="card-header"><div><div class="card-title">Received Early?</div><div class="card-subtitle">Scheduled for the ${s.day}${ord} — log it now if it arrived early</div></div><span class="card-badge" style="background:var(--blue-soft,#e8f0fe);color:var(--blue,#1a73e8)">${fmt(s.amount)}</span></div><div style="font-size:13px;color:var(--text2);line-height:1.7;margin-bottom:12px">Default account: <strong>${esc(acct.name)}</strong>. You can change the date before saving.</div><button class="btn btn-secondary" onclick="openSalaryReceiptModal(${s.day})">💵 Receive Early</button></div>`;}wrap.innerHTML=html;}
